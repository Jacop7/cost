...
- `bash packages/db/scripts/fresh-db.sh fresh_x` 로 새 DB, `--until <14자리>` 로 중간 상태.
  ...
  운영·개발 DB 는 배포 절차에서 `ADMIN_DB_URL=… bash packages/db/scripts/admin-acl.sh postgres fix`
  를 한 번, 이후 `check` 로 게이트한다(CI 는 `--no-db` 라 이 단계를 안 돈다). 시험 31 이
  `pg_default_acl` 로 두 롤을 다 잰다.
