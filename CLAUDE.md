...
- 세금: 부가세 포함 = 판매가 × 10/110. 여기에 **사장님이 더한 세금 항목**(판매가 대비 %)을
  ...
  ⚠ **세금은 매장 하나에 하나다**(0087). 레시피마다 고치지 않는다 — `MY > 세금` 이 유일한 입구이고,
  저장하면 전 레시피 손익이 다시 계산되어 각 메뉴의 손익 변동에 `세금 반영` 한 줄이 남는다
  (고정지출 E4 와 같은 짜임). `recipes.tax_mode/tax_items` 는 그 전파 결과이며 트리거가 채운다 —
  `save_recipe` 로는 못 바꾼다.
