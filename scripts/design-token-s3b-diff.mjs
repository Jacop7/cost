#!/usr/bin/env node
/** S3b 계약 — S4 exact SHA에서 폭이 늘는 known.assignments만 허용한다. */
...
const knownPath = resolve(opt.known ?? join(root, 'scripts/design-token-s3b-known.json'));
...
if (!baseline) {
  const expectedCommit = opt['expect-commit'];
  if (!expectedCommit) fail('--expect-commit=<full SHA>가 필요하다');
  else {
    currentCommit = git(['rev-parse', 'HEAD']).trim();
    if (expectedCommit !== currentCommit) fail(`HEAD ${currentCommit} ≠ expect ${expectedCommit}`);
    const dirty = git(['status', '--porcelain', '--', 'apps/mobile/app', 'apps/mobile/src',
      'scripts/design-token-s3b-known.json', 'scripts/design-token-s3b-diff.mjs']).trim();
    if (dirty) fail(`S3b 결속 범위가 깨끗하지 않다 — ${dirty.split(/\r?\n/).length}건`);
  }
}
const walk = base => {
  const files = [];
...
for (const item of plan.values()) {
  ...
  if (!['H', 'B'].includes(item.axis) || item.target <= item.current)
    fail(`${item.key} S3b 증가 계약 위반 — axis ${item.axis} · ${item.current}→${item.target}`);
}
...
if (JSON.stringify(cssRows(protoBefore)) !== JSON.stringify(cssRows(protoAfter))) fail('S3b에서 프로토타입 CSS 기하가 바뀌었다');

const result = { schemaVersion: 1, stage: 'S3b', baselineCommit, currentCommit, assignments: plan.size,
...
console.log(`S3b 치환 계약 — ${used.size}/${plan.size} · 기하 선언 ${declarations}`);
if (failures.length) { console.error(failures.map(x => `  - ${x}`).join('\n')); process.exit(1); }
console.log('S3b 치환 계약 PASS');
