...
group('SAMPLE_REVIEWED', 'screen:order_main popup:order_candidates@order_main popup:order_waiting@order_main popup:order_received@order_main popup:order_order@order_main popup:order_receive@order_main', 'orders', '발주 target별 현재 검증 상한', '보고서 target 표의 목록·두 시트 웹/host 및 Sol 한정 PASS', '목록별 추가 데이터·키보드·native; ORD-06 registry 경로 정정은 별도');
group('PARTIAL', 'popup:order_cancel@order_main popup:order_revert@order_main', 'orders', 'ORD-07 취소 Alert 경계', 'Alert API/웹 bridge 동작 시험만', '실제 dialog 시각·native');
