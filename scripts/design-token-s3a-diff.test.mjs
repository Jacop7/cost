...
const run = (edit, touchAdjustments = []) => {
  ...
  try {
    ...
    const before = `const x={paddingHorizontal:15,fontSize:15,color:'#000'};\nconst y=<Pressable hitSlop={5}/>;`;
    ...
    const known = { baselineCommit: 'fixture', assignments: 2, files: 1,
      byRule: { A: 1, B: 1 },
      tokenContract: { space: { xs:4,sm:8,md:12,lg:16,xl:20,xxl:24 }, radius:{sm:8,md:12,lg:16,xl:20,full:999}, TYPE:{caption:{fontSize:14,lineHeight:20}}, controlVisualHeight:{sm:32,md:38} },
      assignmentPlan: [
...
test('승인 목적지 대신 다른 토큰을 쓰면 실패한다', () => assert.equal(run(`import {space,TYPE} from './theme/tokens';\nconst x={paddingHorizontal:space.sm,fontSize:TYPE.caption.fontSize,color:'#000'};`).status, 1));
test('승인되지 않은 기하 변경을 함께 넣으면 실패한다', () => assert.equal(run(`import {space,TYPE} from './theme/tokens';\nconst x={paddingHorizontal:space.md,fontSize:TYPE.caption.fontSize,width:40,color:'#000'};`).status, 1));
