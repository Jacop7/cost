...
/** 재료에서 확정한 회색 읽기 전용 결과 규격. 입력 컴포넌트가 아니다. */
export function ResultField({ label, value, negative = false, align = 'right' }: {
  label: string; value: string; negative?: boolean; align?: 'left' | 'right';
}) {
  return <Field label={label} variant="stacked">
    <View style={{ minHeight: COMPONENT.stackedForm.controlMinHeight,
      ...
      <Text style={[TYPE.body, tnum, { fontWeight: '800', textAlign: align, color: negative ? COLOR.status.negative : T.ink }]}>{value}</Text>
    </View>
  </Field>;
}
