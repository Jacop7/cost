...
import { space, T, TYPE } from '@/theme/tokens';
...
export function EmptyState({ title, hint, action }: { title: string; hint?: string; action?: ReactNode }) {
  return (
    <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center', padding: space.xxl }}>
      ...
      {hint ? <Text style={{ marginTop: space.sm, color: T.sub2, textAlign: 'center', lineHeight: TYPE.caption.lineHeight }}>{hint}</Text> : null}
