...
export function useInternationalTaxState(){const storeId=useStoreId();return useQuery({queryKey:qk.internationalTax,enabled:Boolean(storeId),queryFn:async()=>{
  const {data,error}=await supabase.rpc('international_tax_app_state',{p_store:storeId});if(error)throw rpcError(error);return parseInternationalTaxState(data,storeId);
}});}
...
export function useRecipeTaxState(recipeId:string,enabled=true){const storeId=useStoreId();return useQuery({queryKey:qk.recipeTax(recipeId),enabled:Boolean(storeId&&recipeId&&enabled),queryFn:async()=>{
  const {data,error}=await supabase.rpc('recipe_tax_app_state',{p_store:storeId,p_recipe:recipeId});if(error)throw rpcError(error);return parseRecipeTaxState(data,storeId);
}});}
...
},onSuccess:()=>{invalidate(qc,[qk.internationalTax,qk.recipes]);}});}
