/**
 * SALES-11 고정지출 자세히 — 월 고정지출을 이 기간 매출 비중으로 나눈 금액.
 *
...
export default function SalesFixedScreen() {
  return (
    <BusinessDateGate source={useSalesBusinessDate()} title="고정 지출">
      {(serverToday) => <SalesFixedScreenBody serverToday={serverToday} />}
    </BusinessDateGate>
  );
}

function SalesFixedScreenBody({ serverToday }: { serverToday: string }) {
