...
export default function SalesRevenueScreen() {
  return (
    <BusinessDateGate source={useSalesBusinessDate()} title="매출">
      {(serverToday) => <SalesRevenueScreenBody serverToday={serverToday} />}
    </BusinessDateGate>
  );
}

function SalesRevenueScreenBody({ serverToday }: { serverToday: string }) {
