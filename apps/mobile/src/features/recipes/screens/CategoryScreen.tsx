/**
 * RCP-12 레시피 카테고리 설정 — 카테고리 추가·수정·삭제(인라인 편집).
 * ⚠ 디자인 프로토타입(로컬 상태). 실제 저장은 데이터 연결 단계(설정)에서.
 */
import { useState } from 'react';
import { Pressable, ScrollView, Text, View } from 'react-native';
import { AppHeader, Button, Card, Icon, Input } from '@/components/kit';
import { safeBack } from '@/lib/nav';
import { T } from '@/theme/tokens';
import { RECIPE_CATS } from '../demoData';

export default function CategoryScreen() {
  const [cats, setCats] = useState<string[]>(RECIPE_CATS);
  ...
  return (
    <View style={{ flex: 1, backgroundColor: T.bg }}>
      <AppHeader
        ...
        right={
          <Pressable onPress={() => safeBack('/recipes')} hitSlop={6} style={{ paddingHorizontal: 8, paddingVertical: 8 }}>
        ...
        <Card pad={0} style={{ overflow: 'hidden' }}>
          {cats.map((c, i) => {
            ...
            return (
              <View
                key={i}
                style={{ flexDirection: 'row', alignItems: 'center', gap: 10, paddingVertical: 10, paddingLeft: 15, paddingRight: 12, borderBottomWidth: i < cats.length - 1 ? 1 : 0, borderBottomColor: T.line2, backgroundColor: on ? T.blueTint : T.surface }}
              >
                {on ? (
                  <>
