...
export default function RecipeTaxScreen() {
  ...
  const current = quote && market ? {
    ...
    basis: 'tax_inclusive' as const, currency: 'KRW' as const, minor: 0, locale: 'ko-KR',
  ...
  const taxApplied = current?.basis === 'tax_inclusive';
  const appliedRate = quote && market ? (quote.listedTotal > 0 ? (primary?.unroundedAmount ?? 0) / quote.listedTotal * 100 : 0)
    : legacyPrimary?.rate;
  const money = (value: number) => formatMarketMoney(value, current!.currency);
      ...
      <QueryState isLoading={capabilities.isLoading || (enabled && (state.isLoading || recommendation.isFetching)) || (beforeActivation && detail.isLoading)} error={capabilities.error ?? state.error ?? recommendation.error ?? (beforeActivation ? detail.error : null)}
        ...
        {enabled && current && taxApplied ? <>
          ...
          <TaxSummaryCard title="세금 제외 금액">
            ...
            <TaxSummaryRow label="(−) 세금 총액" value={money(current.tax)} />
            <TaxSummaryRow label="소계" value={money(current.net)} last />
          </TaxSummaryCard>
        </> : current && !taxApplied ? <TaxSummaryCard title="세금 계산">
          <TaxSummaryRow label="적용 상태" value="판매가에 세금 별도" last />
        </TaxSummaryCard> : <Text style={{ ...TYPE.body, color: COLOR.text.tertiary }}>현재 적용되는 세금 정보를 확인할 수 없어요.</Text>}
          {current && taxApplied ? <>
            <TaxSummaryCard title="부가세 계산 기준">
                <TaxSummaryRow label="메뉴 가격 기준" value="부가세 포함" />
