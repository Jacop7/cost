// VendorPickerSheet.tsx — 거래처 선택 바텀시트 (재료·구매옵션·발주 공용)
//
      ...
      >
        {(!adding || allowAddAction) ? <ScrollView contentContainerStyle={{ paddingBottom: space.lg }} showsVerticalScrollIndicator={false}>
          {allowNone ? (
            <SelectionRow label="지정 안 함" selected={!value} last={vendors.length === 0}
              ...
              accessibilityLabel={Platform.OS === 'web' && !value ? '구매처 없음, 현재 선택됨' : '구매처 없음'}
            />
          ) : null}
        ...
        {adding ? (
          <View style={{ gap: space.sm, paddingTop: 8, borderTopWidth: 1, borderTopColor: T.line2 }}>
            <Input value={name} onChangeText={setName} placeholder="구매처 이름" accessibilityLabel="새 구매처 이름" returnKeyType="done" onSubmitEditing={add} />
          ...
          <Pressable
            ...
            accessibilityRole="button" accessibilityLabel="구매처 추가"
          ...
          >
            ...
            <Text style={{ fontSize: 16, fontWeight: '700', color: COLOR.text.link }}>구매처 추가</Text>
          </Pressable>
        ) : null}
      </QueryState>
    </Sheet>
