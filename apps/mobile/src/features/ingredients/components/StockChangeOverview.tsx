...
export function StockChangeOverview({ id, name, stock, basePrice, unit, mode, minimumStock, changePreview, disabled = false }: {
  id: string; name: string; stock: number; basePrice: number | null; unit: 'g' | 'ml' | '개'; mode?: StockMode; minimumStock?: number;
  changePreview?: { stock: string; basePrice: string }; disabled?: boolean;
}) {
