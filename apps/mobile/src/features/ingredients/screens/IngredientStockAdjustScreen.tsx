/**
 * ING-04 재고 수정 — 프로토타입(식재료(잔여) 4.7절) 을 kit 컴포넌트로 RN 이식.
 * 미개봉/개봉 개수 직접 보정(실사·E5) · 개봉분 완전소진(로스 0) · 폐기(→E2, 남은 양→실측 로스율 보정) · '곧 소진' 플래그.
 * ⚠ 현재는 디자인 프로토타입(로컬 상태). 실제 보정/폐기/실사는 데이터 연결 단계에서 RPC(E2/E5)로.
 *    절대 원칙: 재고·단가·이력은 E1(입고)·E2/E5(재고 수정)에서만 변경 — 이 화면이 그 출처.
 */
import { useState } from 'react';
import { Modal, Pressable, ScrollView, Text, View } from 'react-native';
import { useLocalSearchParams, useRouter } from 'expo-router';
import { AppHeader, Badge, Button, Icon, Sheet, StatusBadge, Stepper } from '@/components/kit';
import { round } from '@sikjae/core';
import { T } from '@/theme/tokens';
import { getIngredient, perLabel } from '../demoData';

const NUM = { fontVariant: ['tabular-nums' as const] };

export default function IngredientStockAdjustScreen() {
  ...
  if (!g) {
    return (
      <View style={{ flex: 1, backgroundColor: T.bg }}>
        <AppHeader title="재고 수정" onBack={() => router.back()} />
  ...
  const [sealed, setSealed] = useState(g.sealed);
  const [opened, setOpened] = useState(g.opened);
  const soon = g.soon;
  const [discarding, setDiscarding] = useState(false);
  const [consumed, setConsumed] = useState(false); // 완전소진 수행 여부
  const [confirmLoss, setConfirmLoss] = useState(false); // 완전소진 후 로스율 인하 확인 다이얼로그
  ...
  // 폐기 패널 토글 — 개봉분을 버리고 남은 양을 로스로 기록.
  const toggleDiscard = () => {
    if (opened <= 0) return;
    setDiscarding((v) => !v);
    setConsumed(false);
  ...
  // 저장 — 완전소진을 했으면 로스율 인하 제안 다이얼로그를 먼저 띄운다.
  const onSave = () => {
    if (consumed) {
      setConfirmLoss(true);
      return;
  ...
  return (
    <View style={{ flex: 1, backgroundColor: T.bg }}>
      ...
      <ScrollView contentContainerStyle={{ paddingHorizontal: 16, paddingTop: 4, paddingBottom: 24, gap: 11 }}>
        ...
        <View style={{ backgroundColor: T.surface, borderRadius: 16, padding: 16 }}>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
            ...
            <Text style={{ fontSize: 18, fontWeight: '800', letterSpacing: -0.4, color: T.ink }}>{g.name}</Text>
            <View style={{ flex: 1 }} />
            <StatusBadge status={total <= 0 ? 'out' : soon ? 'out' : g.status} sm />
          </View>
          <Text style={[{ fontSize: 14, color: T.sub, marginTop: 10 }, NUM]}>
            현재 잔여 <Text style={{ fontWeight: '800', color: T.ink }}>미개봉 {g.sealed} · 개봉 {g.opened}</Text>
            <Text style={{ color: T.ter }}>  ·  개당 {perLabel(g.unit, g.per)}</Text>
          </Text>
        </View>
        ...
        <View style={{ backgroundColor: T.surface, borderRadius: 16, padding: 16 }}>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, marginBottom: 4 }}>
            <Icon name="ruler" size={18} color={T.sub} />
            <Text style={{ fontSize: 15, fontWeight: '800', color: '#5B6573' }}>개수 보정</Text>
            <Badge tone="blue" sm>실사</Badge>
          </View>

          <View style={{ flexDirection: 'row', alignItems: 'center', paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: T.line2 }}>
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 6, paddingTop: 12, borderTopWidth: 1, borderTopColor: T.line2 }}>
            <Text style={{ fontSize: 14, fontWeight: '700', color: T.sub }}>변경 후 총</Text>
            <View style={{ flex: 1 }} />
      ...
      {/* 하단 액션 — 완전소진 · 폐기 · 저장 한 줄 */}
      <View style={{ flexDirection: 'row', gap: 8, paddingHorizontal: 20, paddingTop: 12, paddingBottom: 30, backgroundColor: T.surface, borderTopWidth: 1, borderTopColor: T.line2 }}>
        <Button kind="gray" size="lg" onPress={consumeFully} style={{ flex: 1, opacity: opened <= 0 ? 0.45 : 1 }}>
          완전소진
        </Button>
        <Button kind="danger" size="lg" onPress={toggleDiscard} style={{ flex: 1, opacity: opened <= 0 ? 0.45 : 1, borderWidth: discarding ? 1.5 : 0, borderColor: T.red }}>
          폐기
        </Button>
        ...
        <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.42)', alignItems: 'center', justifyContent: 'center', paddingHorizontal: 32 }}>
          <View style={{ width: '100%', maxWidth: 340, backgroundColor: T.surface, borderRadius: 18, paddingTop: 22, paddingHorizontal: 20, paddingBottom: 14 }}>
            ...
            <View style={{ flexDirection: 'row', gap: 9, marginTop: 18 }}>
              <Pressable
                onPress={() => { setConfirmLoss(false); router.back(); }}
              ...
              <Pressable
                onPress={() => { setConfirmLoss(false); setLossSheet(true); }}
      ...
      <Sheet visible={lossSheet} onClose={() => setLossSheet(false)} title="로스율 수정" sub="실제 재료 소진 기록을 바탕으로 로스율을 조정합니다.">
        ...
        <Button kind="primary" size="lg" full onPress={() => { setLossSheet(false); router.back(); }}>
          로스율 {lossRate}%로 저장
        </Button>
      </Sheet>
    </View>
