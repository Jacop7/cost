...
function InternationalTaxScreen(){
  ...
  const [loaded,setLoaded]=useState(false);const [defaultTreatment,setDefaultTreatment]=useState<TaxTreatment>('taxable');const [components,setComponents]=useState<TaxComponentInput[]>([]);const [error,setError]=useState<string|null>(null);
  useEffect(()=>{if(loaded||!state.data)return;if(profile){setDefaultTreatment(profile.defaultTreatment);setComponents(profile.components.map((c)=>({key:c.configKey,kind:c.kind,name:c.name,ratePct:c.ratePct,jurisdictionLevel:c.jurisdictionLevel,calculationBasis:c.calculationBasis,appliesToTreatments:[...c.appliesToTreatments],sortOrder:c.sortOrder,remittance:{hall:profile.remittanceRules.find(r=>r.taxComponentId===c.id&&r.salesChannel==='hall')?.remittanceOwner??'merchant',delivery:profile.remittanceRules.find(r=>r.taxComponentId===c.id&&r.salesChannel==='delivery')?.remittanceOwner??'merchant',takeout:profile.remittanceRules.find(r=>r.taxComponentId===c.id&&r.salesChannel==='takeout')?.remittanceOwner??'merchant'}})));}else if(market){setComponents([{key:'primary',kind:'primary',name:market.countryCode==='KR'?'부가세':'Primary tax',ratePct:market.countryCode==='KR'?10:0,jurisdictionLevel:'national',calculationBasis:'primary_tax_exclusive',appliesToTreatments:['taxable'],sortOrder:0,remittance:{hall:'merchant',delivery:'merchant',takeout:'merchant'}}]);}setLoaded(true);},[loaded,state.data,market,profile]);
  ...
  const invalid=!market||components.length===0||components.filter(c=>c.kind==='primary').length!==1||components.some(c=>!c.name.trim()||c.ratePct<0||c.ratePct>=100);
  const update=(key:string,part:Partial<TaxComponentInput>)=>setComponents(rows=>rows.map(r=>r.key===key?{...r,...part}:r));
  ...
  const onSave=()=>{if(invalid||save.isPending||!state.data?.capabilities.internationalTax.writeEnabled)return;setError(null);save.mutate({defaultTreatment,components,categories,baseProfileId:profile?.id??null,baseRevision:profile?.revision??null},{onSuccess:()=>void state.refetch(),onError:(e)=>{if(e instanceof RpcError&&e.code==='45009'){setError('다른 기기에서 세금 설정이 변경됐어요. 새로고침해 주세요.');return;}setError(e instanceof Error?e.message:'저장하지 못했어요');}});};
