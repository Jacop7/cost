  ...
  return <View style={{flex:1,backgroundColor:T.bg}}>
    ...
    <ScrollView contentContainerStyle={{padding:16,paddingBottom:32,gap:12}}>
      <QueryState isLoading={state.isLoading} error={state.error} isEmpty={false} onRetry={()=>void state.refetch()} emptyTitle="국가 정보가 없어요">
        ...
        <Card>
          <Text style={{fontSize:14,fontWeight:'700',color:COLOR.text.tertiary}}>영업 국가</Text>
...
function Choice({selected,label,disabled,onPress}:{selected:boolean;label:string;disabled?:boolean;onPress:()=>void}){return <Pressable accessibilityRole="button" accessibilityLabel={`${label}${selected?' 선택됨':''}`} accessibilityState={{selected,disabled}} disabled={disabled} onPress={onPress} style={{paddingHorizontal:12,paddingVertical:9,borderRadius:10,borderWidth:1,borderColor:selected?COLOR.action.primary:T.line2,backgroundColor:selected?COLOR.action.primaryTint:T.surface}}><Text style={{fontWeight:'700',color:selected?COLOR.state.selectedText:T.ink2}}>{label}</Text></Pressable>;}
