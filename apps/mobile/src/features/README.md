...
### 국제 출시 앱 개정 후보 (`INTL-1E`, 화면 구현 전)

[국가·통화·세금 국제 출시 기획안](../../../../docs/국가-통화-세금-국제출시-기획안.md)이 다음 화면
계약을 정의한다. 아래 항목은 현재 ✅ 인벤토리를 바꾸는 구현 완료 표시가 아니다.

- `MY-02`: 국가, 기본세 포함·미포함, 국가별 기본세율, 과세 상태, 납부 주체, 추가세별 계산 기준
- `MY-08`: 앱 언어를 매장 국가·업무 로케일·통화와 분리. 영어 UI가 `USD`를 강제하지 않음
...
현재 구현의 `로케일 → 통화` 및 판매가 대비 유효 세율 계약은 `INTL-1E` 앱 전환과 별도 capability
활성화가 완료되기 전까지 유지한다. `0179`~`0181`의 비활성 schema·감사·계산 몸통만으로 화면 계약이
바뀌지는 않는다.
