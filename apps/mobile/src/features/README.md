...
| `my` | MY-12 | 국가·통화 확인 (국제 세금 capability 전에는 읽기 전용) | `my/country` (`MyCountryScreen`) | ✅ |

> 위 ✅ 는 **실데이터 연결 + 전파 + 재조회**까지 구현된 현재 인벤토리다(2026-08-29).

### 국제 출시 앱 개정 (`INTL-1E`, 비활성 계약 연결 중)

[국가·통화·세금 국제 출시 기획안](../../../../docs/국가-통화-세금-국제출시-기획안.md)이 다음 화면
계약을 정의한다. 아래 항목은 현재 ✅ 인벤토리를 바꾸는 구현 완료 표시가 아니다.

- `MY-02`: capability가 열리면 국가별 기본세·추가세·납부 주체를 프로필 판본으로 표시한다. 비활성 동안 구 편집기를 유지한다
- `MY-08`: 앱 언어를 사용자별 `ko/en`으로 분리했다. 영어 UI가 `USD`를 강제하지 않는다
...
국제 세금 read/write capability는 계속 꺼져 있어 현재 판매 계산과 MY-02 저장은 `0090` 계약을 유지한다.
`0182`는 사용자 언어와 비활성 앱 조회 계약만 연결하며, 실제 새 세금 쓰기는 `INTL-1F` cutover 전에는 열지 않는다.
