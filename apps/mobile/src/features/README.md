...
2026-09-13 재료 통합: 별도 부자재 관리·검색·추가·수정 라우트는 재료 관리·검색으로 연결한다. 신규 메뉴는 재료 lines만 저장한다. 재고 미관리 재료는 기준 단가와 메뉴 비용을 제공하고 재고 입력·부족 상태는 표시하지 않는다. 통합 전 수정 내역은 원본 그대로 조회한다. 사용자 표기는 재료이며 ING/RCP와 내부 URL 식별자는 유지한다.
...
| `ingredients` | RCP-13b | 통합 재료 관리 (재고 관리 사용/미사용 재료의 이름·카테고리·기준 단가, 등록·수정·삭제 연결) | `recipes/ingredients` (`IngredientManageScreen`) | ✅ |
...
| `recipes`→`my` | RCP-15 | 적용 채널·비중 (고정 지출 수정 내 시트·슬라이더·합계 검증) | `my/ChannelWeightSheet`(시트) | ✅ |
...
| `orders` | ORD-03 | 입고 완료 (실제 수량·부분 입고·멱등키) → **E1** | (OrdersHome 내 시트) | ✅ |
...
| `sales` | SALES-05b | 판매 수량 입력 (매장/배달/포장 + **조리 후 폐기**) → **E10/E8** | (SalesHome 내 시트) | ✅ |
