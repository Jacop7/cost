...
export default function ConfigurationHistoryScreen() {
  ...
  const kind = params.kind === 'fixed_cost' ? 'fixed_cost' : params.kind === 'material' ? 'material' : 'tax';
  ...
  const title = kind === 'tax' ? '세금' : kind === 'material' ? '부자재' : `고정 지출${month ? ` · ${month}` : ''}`;
  return <View style={{ flex: 1, backgroundColor: T.bg }}>
    <AppHeader title="수정 내역" onBack={() => safeBack(kind === 'tax' ? '/my/tax' : kind === 'material' ? '/recipes/materials' : `/recipes/fixed-cost${month ? `?month=${month}` : ''}`)} />
          ...
          {items.map((event, index) => <Pressable key={event.id} accessibilityRole="button" accessibilityLabel={`${event.title} ${changeStamp(event.occurredAt, timezone)}`} onPress={() => setSelected(event)}
            ...
            <View style={{ flex: 1, gap: space.xs }}>
              ...
              <View style={{ flexDirection: 'row', alignItems: 'center', flexWrap: 'wrap', gap: space.xs }}>
                <ChangeSourceBadge automatic={false} />
                <Text style={{ ...TYPE.body, color: COLOR.text.primary, flexShrink: 1 }}>{event.title}</Text>
              </View>
              ...
              {event.effectiveFrom || (kind === 'material' && event.applicationMode) ? <Text style={{ ...TYPE.captionSm, color: COLOR.text.tertiary }}>{event.applicationMode === 'immediate' ? '즉시 적용' : event.applicationMode === 'next_business' ? '영업 종료 후 적용' : `${event.effectiveFrom}부터 적용`}</Text> : null}
    ...
    <Sheet visible={selected !== null} title="수정 내용" onClose={() => setSelected(null)}>
      {selected ? <View style={{ paddingBottom: space.xl, gap: space.sm }}>
        ...
        {selected.effectiveFrom || (kind === 'material' && selected.applicationMode) ? <Text style={{ ...TYPE.captionSm, color: COLOR.text.secondary }}>{selected.applicationMode === 'immediate' ? '즉시 적용' : selected.applicationMode === 'next_business' ? '영업 종료 후 적용' : `${selected.effectiveFrom}부터 적용`}</Text> : null}
