...
- 확정 계산은 DB RPC, 앱 미리보기는 `@costkeep/core`를 사용한다.
- 저장은 구매단위·N인분 입력을 g/ml/개·1인분 기준으로 환산한 뒤 수행한다.
- 서버 날짜를 받기 전 기기 시계로 영업일을 추정하지 않는다.
- 재고 상태는 `@costkeep/core`의 `stockStateOf` 한 곳에서 판정한다.
...
```bash
# 앱만 빠르게
corepack pnpm --filter @costkeep/mobile typecheck
corepack pnpm --filter @costkeep/mobile test
