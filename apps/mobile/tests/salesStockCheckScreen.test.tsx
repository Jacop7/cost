...
const result = (mode: ShortageResult['mode'] = 'start'): ShortageResult => ({ mode, hasBasis: true, ingredientCount: 8,
  recipes: ['메뉴A', '메뉴B'].map(recipeId => ({ recipeId, name: recipeId,
    ingredients: [1, 2, 3, 4].map(n => ingredient(`${recipeId}-재료${n}`)) })) });
...
describe('부족 재고 전체 보기의 서버 판정 연결', () => {
  ...
  it('판매 묶음·서버 증가분 필요량을 유지하며 최소재고가 충분해도 판매 부족 재료를 숨기지 않는다', () => {
    ...
    const row = within(screen.getByRole('button', { name: '메뉴A-재료4 입고' }));
  ...
  it('필터는 재조회한 서버 재고를 반영하고 전체 재료 보기로 해제할 수 있다', () => {
  ...
  it('필터 진입은 유지하되 재료 탭을 직접 누른 일반 재진입은 잔류 필터를 지운다', () => {
    m.stock = 'below-safety'; const view = render(<IngredientListScreen />);
  ...
  it('여러 메뉴의 합산 부족은 메뉴별 필요량이 현재 재고보다 작아도 서버 결과를 그대로 표시한다', () => {
    ...
    data.recipes = ['메뉴A', '메뉴B'].map(recipeId => ({ recipeId, name: recipeId,
      ingredients: [ingredient('공유재료', { need: 300, stock: 500, safetyStock: 10 })] }));
    m.sale.mockReturnValue(query(data)); render(<SalesStockCheckScreen />);
    expect(screen.getAllByRole('button', { name: '공유재료 입고' })).toHaveLength(2);
    for (const button of screen.getAllByRole('button', { name: '공유재료 입고' })) {
      expect(within(button).getByText(/필요 수량/).textContent).toContain('300g');
    }
  });
  it('메뉴별 더보기·접기·입고는 기존 필요량 판정을 유지한다', () => {
    render(<SalesStockCheckScreen />);
    fireEvent.click(screen.getAllByRole('button', { name: '재료 1개 더 보기' })[0]!);
  ...
  it('판매 딥링크에 묶음이 없으면 시작 판정만 사용하고 음수 재고를 보존한다', () => {
    m.mode = 'sale'; const data = result(); data.recipes[0]!.ingredients[0] = ingredient('음수재료', { stock: -750 });
    ...
    expect(within(screen.getByRole('button', { name: '음수재료 입고' })).getByText(/현재 재고/).textContent).toContain('−750g');
  });
  it('기본 목록과 알 수 없는 필터는 기존 전체 목록을 유지하며 필터의 검색·빈 상태를 구분한다', () => {
    ...
    fireEvent.change(screen.getByPlaceholderText('재료·카테고리·구매처 검색'), { target: { value: '안전선' } });
    expect(visibleRows()).toEqual(['안전선동일 상세', '안전선미달 상세']);
    fireEvent.change(screen.getByPlaceholderText('재료·카테고리·구매처 검색'), { target: { value: '판매부족만' } });
    expect(screen.getByText("'판매부족만' 검색 결과가 없어요")).toBeTruthy();
    fireEvent.change(screen.getByPlaceholderText('재료·카테고리·구매처 검색'), { target: { value: '' } });
