...
function Host({action}:{action:()=>void}) { const gate=useBusinessEditConfirmation('재료');return <>{gate.dialog}<button onClick={()=>gate.request(action)}>편집 열기</button></>; }
...
it.each(['open','break'])('%s: 확인 전에는 편집을 열지 않고 취소/확인을 구분한다',status=>{
  ...
  expect(screen.getByText('재료를 수정하시겠습니까?')).toBeTruthy();expect(screen.getByText(BUSINESS_EDIT_MESSAGE)).toBeTruthy();
