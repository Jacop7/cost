...
const addDraft = (): Partial<RecipeDraft> => ({
  ...
  lines: [
    ...
    { ingredientId: 'material-box', subRecipeId: null, name: '용기', unit: '개', inputQty: 20, unitPrice: 300 },
  ],
  extras: [],
});
...
describe('RCP-03/04 실제 레시피 폼 배치·초안·저장 계약', () => {
  ...
  it('공용 검색 액션 이름을 유지하고 재료·부자재 route로 이동한다', () => {
    ...
    render(<RecipeAddScreen />);
    fireEvent.click(screen.getByRole('button', { name: '재료 추가' }));
    expect(screen.queryByRole('button', { name: '부자재 추가' })).toBeNull();
  ...
  it('재료비 소계는 서버 단가 초안의 1인분 값이고 DOM에서 재료 검색보다 먼저다', () => {
    useRecipeDraft.getState().reset(addDraft());
    render(<RecipeAddScreen />);
    expect(within(subtotalRow()).getByText('1,100원')).toBeTruthy();
    const subtotal = screen.getAllByText('소계')[0]!;
    const search = screen.getByRole('button', { name: '재료 추가' });
  ...
  it('공용 ScrollTabs의 기준 인분·1인분 전환이 재료 줄과 소계 금액에 같이 반영된다', () => {
    ...
    expect(within(subtotalRow()).getByText('1,100원')).toBeTruthy();
    expect(within(screen.getByRole('button', { name: '대파 재료 사용량 수정' })).getByText('400원')).toBeTruthy();
    fireEvent.click(batch);
    expect(batch.getAttribute('aria-selected')).toBe('true');
    expect(within(subtotalRow()).getByText('11,000원')).toBeTruthy();
    expect(within(screen.getByRole('button', { name: '대파 재료 사용량 수정' })).getByText('4,000원')).toBeTruthy();
    fireEvent.click(one);
    expect(one.getAttribute('aria-selected')).toBe('true');
    expect(within(subtotalRow()).getByText('1,100원')).toBeTruthy();
  });
  ...
  it('legacy 월평균 초안값이 있어도 입력과 월평균 손익 탭을 다시 노출하지 않는다', () => {
    ...
    expect(screen.getAllByRole('tab', { name: /인분$/ })).toHaveLength(8);
  ...
  it('사용량 수정 시트는 기준 인분 전체 수량과 소계를 같이 바꾼다', () => {
    useRecipeDraft.getState().reset(addDraft());
    render(<RecipeAddScreen />);
    fireEvent.click(screen.getByRole('button', { name: '대파 재료 사용량 수정' }));
    expect(modal().getByText('재료 사용량 수정')).toBeTruthy();
    ...
    expect(within(screen.getByRole('button', { name: '대파 재료 사용량 수정' })).getByText('150g · 4.00원/g')).toBeTruthy();
    expect(within(subtotalRow()).getByText('1,300원')).toBeTruthy();
    expect(mock.save).not.toHaveBeenCalled();
  });
  ...
  it('추가 저장은 기존 숫자 변환·trim·line·extra payload를 그대로 도메인 hook에 넘긴다', () => {
    ...
    expect(mock.save).toHaveBeenCalledWith({
      ...
      lines: [
        ...
        { ingredientId: 'material-box', subRecipeId: null, inputQty: 20 },
      ],
      extras: [],
  ...
  it('추가 페이지 전체의 기본 입력·재료·부자재·손익·하단 행동이 확정안 순서다', () => {
    ...
    expect(screen.queryByText(/고정 지출률은 저장 후/)).toBeNull();
    ...
    expect(screen.queryByText('소계')).toBeNull();
    const ordered = [screen.getByText('등록된 재료가 없습니다.'), screen.getByRole('button', { name: '재료 추가' }),
      screen.getByText('판매 손익'), screen.getByRole('button', { name: '메뉴 등록' })];
    ordered.slice(1).forEach((el, i) => expect(ordered[i]!.compareDocumentPosition(el) & Node.DOCUMENT_POSITION_FOLLOWING).not.toBe(0));
    for (const label of ['재료 추가']) {
    ...
    expect(screen.getByRole('button', { name: '메뉴 등록' }).getAttribute('aria-disabled')).toBe('true');
  });

  it('등록 카테고리만 선택하며 필수 항목이 준비돼야 저장한다', () => {
    render(<RecipeAddScreen />);
    fill('메뉴명', '검수 메뉴'); fill('판매가', '12000');
    const submit = screen.getByRole('button', { name: '메뉴 등록' });
  ...
  it('부자재가 한 항목이면 소계 없이 행에 수량을 반영한 1인분 금액·판매가 대비 비율이며 메모 데이터는 숨겨도 보존한다', () => {
    useRecipeDraft.getState().reset(addDraft()); render(<RecipeAddScreen />);
    const subtotal = within(screen.getByRole('button', { name: '용기 재료 사용량 수정' }));
    ...
    fireEvent.click(screen.getByRole('button', { name: '용기 재료 사용량 수정' }));
    fireEvent.change(modal().getByRole('textbox', { name: '사용량' }), { target: { value: '30' } });
    fireEvent.click(modal().getByRole('button', { name: '저장' }));
  ...
  it('재료 행은 사용량·비율과 화살표를 표시하고 삭제는 팝업 안에서만 초안에 반영한다', () => {
    useRecipeDraft.getState().reset(addDraft()); render(<RecipeAddScreen />);
    const row = screen.getByRole('button', { name: '대파 재료 사용량 수정' });
    ...
    expect(useRecipeDraft.getState().draft.lines.map(l => l.name)).toEqual(['소스','용기']);
    expect(mock.save).not.toHaveBeenCalled();
  });
  ...
  it('부자재 소수 사용량은 1인분 기준이며 닫기·삭제·저장이 실제 레시피 저장과 분리된다', () => {
    useRecipeDraft.getState().reset(addDraft()); render(<RecipeAddScreen />);
    fireEvent.click(screen.getByRole('button', { name: '용기 재료 사용량 수정' }));
    fireEvent.change(modal().getByRole('textbox', { name: '사용량' }), { target: { value: '2.5' } });
    ...
    expect(useRecipeDraft.getState().draft.lines[2]?.inputQty).toBe(20);
    fireEvent.click(screen.getByRole('button', { name: '용기 재료 사용량 수정' }));
    fireEvent.change(modal().getByRole('textbox', { name: '사용량' }), { target: { value: '0' } });
    expect(modal().getByRole('button', { name: '저장' }).getAttribute('aria-disabled')).toBe('true');
    fireEvent.change(modal().getByRole('textbox', { name: '사용량' }), { target: { value: '2.5' } });
    fireEvent.click(modal().getByRole('button', { name: '저장' }));
    expect(useRecipeDraft.getState().draft.lines[2]?.inputQty).toBe(2.5);
    fireEvent.click(screen.getByRole('button', { name: '용기 재료 사용량 수정' }));
    fireEvent.click(modal().getByRole('button', { name: '삭제' }));
    expect(useRecipeDraft.getState().draft.lines.map(l => l.name)).toEqual(['대파','소스']);
    expect(mock.save).not.toHaveBeenCalled();
  });
});
