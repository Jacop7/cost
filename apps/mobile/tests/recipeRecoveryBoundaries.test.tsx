...
it('keeps a create receipt after blur and resolves the same key on refocus before a second create can be sent', async () => {
  ...
  await waitFor(() => expect(screen.getByRole('button', { name: '메뉴 등록' })).toHaveProperty('disabled', false));
  ...
  await act(async () => {});
  await act(async () => { fireEvent.click(screen.getByRole('button', { name: '메뉴 등록' })); });
  ...
  expect(screen.getByRole('button', { name: '메뉴 등록' })).toHaveProperty('disabled', true);
  fireEvent.click(screen.getByRole('button', { name: '메뉴 등록' })); expect(calls).toBe(1);
...
it.each(['add', 'detail', 'memo'] as const)('surfaces unreadable journal discard failure in the %s UI and keeps writes blocked', async surface => {
  ...
  if (surface === 'add') expect(screen.getByRole('button', { name: '메뉴 등록' })).toHaveProperty('disabled', true);
