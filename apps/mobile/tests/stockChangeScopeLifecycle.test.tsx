...
vi.mock('@/features/ingredients/hooks',()=>({useInventoryOccurrenceContext:()=>({data:{requiresConfirmation:false},isLoading:false,error:null,refetch:vi.fn()}),useStockChange:()=>({mutate:m.save,isPending:false,hasPending:async()=>false,resolvePending:async()=>null}),useIngredientDetail:()=>({data:{id:m.id,name:'대파',stockTotal:1000,basePrice:4,baseUnit:'g'},isLoading:false,isSuccess:true,isFetching:false,isFetchedAfterMount:true,error:null,refetch:vi.fn()})}));
