...
const selectLanguage=(name:'한국어'|'English')=>{fireEvent.click(screen.getByLabelText('앱 언어 선택'));fireEvent.click(screen.getByLabelText(`${name} 선택`));};
...
describe('사용자 앱 언어',()=>{
  ...
  it('한국어·영어만 고르고 통화는 바꾸지 않는다고 설명한다',()=>{
    render(<MyLanguageScreen/>);expect(screen.getByLabelText('앱 언어 선택').textContent).toBe('한국어');fireEvent.click(screen.getByLabelText('앱 언어 선택'));expect(screen.getByLabelText('한국어 선택').getAttribute('aria-pressed')).toBe('true');expect(screen.getByText(/지금은 언어 선호만 저장해요/)).toBeTruthy();expect(screen.getAllByText(/화면 번역은 준비 중이에요/).length).toBeGreaterThan(0);expect(screen.getByText(/매장 국가·통화·세금·시간대는 바뀌지 않아요/)).toBeTruthy();expect(screen.queryByText('日本語')).toBeNull();
  });
  it('확인 필요 이관은 한국어를 기본 선택하고 저장할 수 있다',()=>{
    preferenceQuery.mockReturnValue(loaded(null));render(<MyLanguageScreen/>);expect(screen.getByText(/자동으로 옮길 수 없었어요/)).toBeTruthy();expect(screen.getByLabelText('앱 언어 선택').textContent).toBe('한국어');expect(disabled('저장')).toBe(false);
  });
  it('편집 기준 판본과 사용자 언어만 저장하고 성공 뒤 이동한다',async()=>{
    render(<MyLanguageScreen/>);selectLanguage('English');fireEvent.click(screen.getByLabelText('저장'));fireEvent.click(screen.getByLabelText('앱 언어 저장 확정'));
  ...
  it('실패는 화면 안에 남고 저장 중에는 이탈·연타를 막는다',async()=>{
    const {rerender}=render(<MyLanguageScreen/>);selectLanguage('English');fireEvent.click(screen.getByLabelText('저장'));pending=true;rerender(<MyLanguageScreen/>);fireEvent.click(screen.getByLabelText('앱 언어 저장 확정'));fireEvent.click(screen.getByLabelText('앱 언어 저장 확정'));expect(mutate).toHaveBeenCalledTimes(0);
  ...
  it('45009 뒤에는 더 높은 판본을 실제로 받아야 잠금이 풀린다',async()=>{
    ...
    render(<MyLanguageScreen/>);selectLanguage('English');fireEvent.click(screen.getByLabelText('저장'));fireEvent.click(screen.getByLabelText('앱 언어 저장 확정'));callbacks().onError(new RpcError('충돌','45009','REVISION_CONFLICT'));
    await waitFor(()=>expect(screen.getByRole('status')).toBeTruthy());fireEvent.click(screen.getByText('새로고침'));await waitFor(()=>expect(q.refetch).toHaveBeenCalledTimes(1));expect(screen.getByRole('status')).toBeTruthy();fireEvent.click(screen.getByText('새로고침'));await waitFor(()=>expect(q.refetch).toHaveBeenCalledTimes(2));expect(screen.queryByRole('status')).toBeNull();expect(screen.getByLabelText('앱 언어 선택').textContent).toBe('English');
  });
  it('배경에서 수락한 새 언어를 이후 변경 비교 기준으로 쓴다',async()=>{
    ...
    await waitFor(()=>expect(screen.getByLabelText('앱 언어 선택').textContent).toBe('English'));
    selectLanguage('한국어');
