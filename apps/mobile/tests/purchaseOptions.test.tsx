...
describe('구매 옵션 표시와 편집 계약', () => {
  it('스킴 없는 주소는 HTTPS로 보완하고 위험한 스킴은 차단한다', () => {
    for (const input of ['example.com/shop?q=1#item', 'www.example.com', 'example.com:8080/shop', '쇼핑.한국/상품']) {
      expect(normalizePurchaseUrl(input)).toBe(`https://${input}`);
    }
  ...
  for (const input of ['example.com', 'www.example.com', 'https://example.com', 'http://example.com/']) {
    it(`상품 경로 없는 ${input}도 저장 요청에 전달한다`, () => {
      ...
      fireEvent.change(screen.getByLabelText('구매 링크 주소'), { target: { value: input } });
      const save = screen.getByRole('button', { name: '저장' });
      expect(save.getAttribute('aria-disabled')).not.toBe('true');
      fireEvent.click(save);
      expect(mock.save).toHaveBeenCalledWith(expect.objectContaining({ url: input.startsWith('http') ? input : `https://${input}` }), expect.any(Object));
    });
  }
  for (const variant of ['management', 'detail'] as const) {
    it(`${variant}: 공유 행은 긴 이름·숫자 원문과 기존 타이포 변형을 유지한다`, () => {
      ...
      render(<PurchaseOptionRow variant={variant} name={name} seller="아주 긴 구매처 이름" amount="987,654,321원" quantity="1kg" unitPrice="987,654.32원/g" badge="low" hasLink={variant === 'management'} last onPress={press} />);
      const row = screen.getByRole('button', { name: `${name} 수정` });
      fireEvent.click(row); expect(press).toHaveBeenCalledOnce();
      const visibleTitle = variant === 'management' ? '아주 긴 구매처 이름' : name;
      ...
      expect(mock.textStyles.get('987,654,321원')?.fontSize).toBe(variant === 'management' ? 16 : 14);
      const seller = within(row).getByText('아주 긴 구매처 이름');
      expect(within(row).getByText(variant === 'management' ? '1kg' : '987,654,321원').previousElementSibling).toBe(variant === 'management' ? seller.parentElement : seller);
  ...
  it('목록의 최저/최고와 브랜드 우선 표시는 기존 계산을 유지한다', () => {
    ...
    expect(low.getByText('최저')).toBeTruthy(); expect(low.getByText('4.00원/g')).toBeTruthy(); expect(low.getByText('첫 구매처')).toBeTruthy();
    ...
    expect((screen.getByLabelText('상품명') as HTMLInputElement).value).toBe('대파 1kg');
  ...
  it('구매처는 공용 Select 이름/펼침/선택 후 닫기를 연결한다', () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: '구매처 변경, 첫 구매처', expanded: false }));
    expect(screen.getByRole('button', { name: '구매처 변경, 첫 구매처', expanded: true })).toBeTruthy();
    fireEvent.click(screen.getByRole('button', { name: '새 구매처' }));
    expect(screen.getByRole('button', { name: '구매처 변경, 새 구매처', expanded: false })).toBeTruthy();
  ...
  it('추가 폼은 빈 값 저장을 막고 kg 환산을 mock 저장 인자에만 전달한다', () => {
    ...
    fireEvent.change(screen.getByLabelText('상품명'), { target: { value: ' 새 옵션 ' } });
    ...
    expect((screen.getByLabelText('용량') as HTMLInputElement).value).toBe('0.002');
    fireEvent.change(screen.getByLabelText('용량'), { target: { value: '2' } });
    ...
    for (const value of ['', 'javascript:alert(1)', 'file:///secret', 'bad-link']) {
      fireEvent.change(screen.getByLabelText('구매 링크 주소'), { target: { value } });
      fireEvent.click(screen.getByRole('button', { name: '추가' })); expect(mock.save).not.toHaveBeenCalled();
    }
    fireEvent.change(screen.getByLabelText('구매 링크 주소'), { target: { value: ' example.invalid/shop ' } });
    fireEvent.click(screen.getByRole('button', { name: '추가' }));
  ...
  for (const kind of ['loading', 'error', 'missing', 'empty'] as const) {
    it(`${kind}: 공용 조회 상태/빈 옵션 안내가 편집 UI를 잘못 노출하지 않는다`, () => {
      ...
      render(<PurchaseOptionScreen />);
      expect(screen.getByText(kind === 'loading' ? '불러오는 중이에요' : kind === 'error' ? '정보를 불러오지 못했어요' : kind === 'missing' ? '재료를 찾을 수 없어요' : '등록된 구매 옵션이 없어요')).toBeTruthy();
  ...
  it('재료 ID가 없으면 조회 결과와 무관하게 등록 진입을 막는다', () => {
    mock.params = {}; render(<PurchaseOptionScreen />);
    expect(screen.getByText('재료를 먼저 저장해 주세요')).toBeTruthy(); expect(screen.queryByLabelText('상품명')).toBeNull();
  });
});
