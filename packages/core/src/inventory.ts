 ...
 *     앱     `재고 <= 0` → out(소진), `soonOut || 최소재고 미달` → low(소진 임박)
 ...
 *
 * ⚠ 두 번 틀렸던 자리라 기록해 둔다.
 *   ① 3단계를 2단계로 줄이며 '부족'을 **'여유'로** 흡수 → 최소재고 미달인데
 *      '여유'라고 썼다(실측 7종. 애호박 720/1500).
 *   ② 흡수 방향을 뒤집어 '부족'을 **'소진 임박'으로** 보냈다 → 진간장이
 *      최소재고의 99%(1,780/1,800) 인데 '소진 임박'이 됐다. 과장이다.
 *
...
export interface StockSnapshot {
  ...
  stockTotal: number;
  /** 최소재고. ⚠ 0073 이후 **기준단위**다. 개당 용량을 곱하지 않는다. */
  safetyStock: number;
 ...
 * 최소재고 아래인가.
 *
 * ⚠ 경계는 **이하**(`<=`)다. 기획안 §3: "최소재고 이하이면 소진 임박".
 *   앱 쪽 복사본만 `<` 였다 — 최소재고와 정확히 같은 재료를 '여유'로 칠했다.
 *   최소재고는 "이만큼은 있어야 한다"는 선이지 "이만큼이면 넉넉하다"가 아니다.
 *
 * ⚠ 최소재고는 **기준단위**다(0073) — perVolume 을 곱하지 않는다.
