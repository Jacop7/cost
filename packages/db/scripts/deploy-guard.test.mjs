...
ok('production 환경 변수 이름', () => assert.equal(expectedRefEnv('production'), 'COSTKEEP_PRODUCTION_PROJECT_REF'));
ok('staging 환경 변수 이름', () => assert.equal(expectedRefEnv('staging'), 'COSTKEEP_STAGING_PROJECT_REF'));
