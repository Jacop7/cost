...
export function expectedRefEnv(target) {
  return target === 'production' ? 'COSTKEEP_PRODUCTION_PROJECT_REF' : 'COSTKEEP_STAGING_PROJECT_REF';
}
...
export function validateDeployContext({ target, mode, expectedRef, linkedRef, approvedSha, headSha,
  ...
  if (!SHA_RE.test(approvedSha ?? '')) fail('COSTKEEP_APPROVED_DEPLOY_SHA가 없거나 40자리 SHA가 아닙니다.');
...
async function api(path) {
  const response = await fetch(`https://api.github.com/repos/${repository()}${path}`, {
    headers: { Accept: 'application/vnd.github+json', Authorization: `Bearer ${githubToken()}`,
      'User-Agent': 'costkeep-deploy-guard', 'X-GitHub-Api-Version': '2022-11-28' },
  });
...
async function main() {
  ...
  const context = validateDeployContext({
    ...
    approvedSha: process.env.COSTKEEP_APPROVED_DEPLOY_SHA,
    headSha,
  ...
  if (mode === 'apply') {
    ...
    if (process.env.COSTKEEP_DEPLOY_CONFIRM !== expected) {
      fail(`실제 적용 확인값이 다릅니다. 계획 검토 후 COSTKEEP_DEPLOY_CONFIRM=${expected} 를 명시하세요.`);
    }
