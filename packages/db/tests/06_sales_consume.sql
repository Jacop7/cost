...
begin
  ...
  perform pg_temp.e10(pg_temp.store(), v_day, v_rcp, 15, 0, 0, 0);
  perform pg_temp.eq('10→15 수정은 차이 5인분만 반영',
    b_stock - stock_total_base(pg_temp.ing('대파')), v_need * 15, 0.0001);

  -- ── 수량을 3개로 줄이면 되돌아온다 ───────────────────────────
  perform pg_temp.e10(pg_temp.store(), v_day, v_rcp, 3, 0, 0, 0);
  ...
  perform pg_temp.e10(pg_temp.store(), v_day, v_rcp, 5, 0, 0, 2);
