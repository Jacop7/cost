...
begin
  ...
  perform pg_temp.eq('미국 51개·캐나다 13개 1차 관할을 앱 facade가 그대로 읽는다',
    jsonb_array_length(international_tax_regions(pg_temp.store(),'US'))
      + jsonb_array_length(international_tax_regions(pg_temp.store(),'CA')),64,0);
