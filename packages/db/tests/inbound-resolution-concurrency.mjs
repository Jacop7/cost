...
for(const order of ['inbound-first','resolution-first']) {
  ...
  const inbound=`select quick_inbound('${store}','${ingredient}',1000,4000,1,null,null,'${key}');`;
  const resolve=`select resolve_quick_inbound('${store}','${ingredient}','${key}');`;
  const guardedInbound=`do $$ begin perform quick_inbound('${store}','${ingredient}',1000,4000,1,null,null,'${key}'); raise exception 'late inbound was accepted'; exception when sqlstate '45010' then null; end $$;`;
  ...
  const events=Number(q(`select count(*) from inventory_events where store_id='${store}' and idempotency_key='${key}';`));
  ...
  console.log(`PASS ${order}: observed session lock, ledger=${events}, no duplicate or delayed inbound`);
}
