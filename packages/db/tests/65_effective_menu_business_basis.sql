...
begin
 foreach method in array array['manual'::public.business_close_method,'auto'::public.business_close_method] loop
  ...
  perform pg_temp.ok(method||' 영업 전 수정 즉시 반영',public.recipe_detail(r)->>'application_mode'='immediate' and (public.recipe_detail(r)->>'material_cost')::numeric=700);
  set local role postgres;
  ...
  perform pg_temp.ok(method||' 편집에는 최신 저장값이 보존된다',(q->>'price')::numeric=15000 and (q->>'material_cost')::numeric=2300 and (q->>'extra_cost')::numeric=0);
  perform pg_temp.ok(method||' 재고 입고는 대기하지 않는다',public.stock_total_base(ing)=2000);
  perform pg_temp.ok(method||' 상세 원가 손익은 시작 기준을 사용',q->>'application_mode'='after_close' and (q#>>'{effective,price}')::numeric=12000 and (q#>>'{effective,material_cost}')::numeric=700 and (q#>>'{effective,extra_cost}')::numeric=0 and (q#>>'{effective,fixed_rate}')::numeric=0.2 and exists(select 1 from jsonb_array_elements(q#>'{effective,lines}') x where x->>'ingredient_id'=ing::text and (x->>'stock_total')::numeric=2000));
  ...
  perform pg_temp.ok(method||' 목록과 세금도 시작 기준을 사용',(select price=12000 and material_cost=700 and extra_cost=0 and fixed_cost=2400 and tax=1091 from public.recipe_list(s) where id=r) and (public.recipe_tax_app_state(s,r)#>>'{quote,tax_total}')::numeric=1091);
  if method='auto' then
  ...
  perform pg_temp.ok(method||' 판매가 시뮬레이션의 원가도 유지',(q#>>'{one,material}')::numeric=700 and (q#>>'{one,extra}')::numeric=0 and (q#>>'{one,fixed}')::numeric=2400 and (q->>'base_servings')::numeric=10);
  q:=public.recipe_price_recommendation(s,r);
  perform pg_temp.ok(method||' 추천 견적은 상세와 동일한 기준',(q#>>'{one,material}')::numeric=700 and (q#>>'{one,extra}')::numeric=0 and (q#>>'{one,fixed}')::numeric=2400);
  set local role postgres;
  if method='auto' then
  ...
  perform pg_temp.as_owner(u);
  q:=public.recipe_detail(r);
  perform pg_temp.ok(method||' 종료하면 추가 저장 없이 최신 기준으로 전환',q->>'application_mode'='immediate' and not(q?'effective') and (select price=15000 and material_cost=2300 and extra_cost=0 and fixed_cost=4500 and tax=2500 and profit=5700 from public.recipe_list(s) where id=r));
  set local role postgres;
