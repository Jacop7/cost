...
begin
  foreach stage in array array['before_open','open','break','closed'] loop
    ...
    p:=jsonb_set(p,'{default_treatment}','"exempt"');
    v:=public.save_store_tax_profile(s,p,(v->>'profile_id')::uuid,(v->>'revision')::integer);
    perform pg_temp.ok(stage||': 기본값 변경 후 상속·미선택은 면세, 명시 과세·분류는 보존',
      (public.pending_recipe_tax_quote(inherited)->>'tax_total')::numeric=0
      and (public.pending_recipe_tax_quote(untouched)->>'tax_total')::numeric=0
      and (public.pending_recipe_tax_quote(explicit_taxable)->>'tax_total')::numeric=1000
      and (public.pending_recipe_tax_quote(category_menu)->>'tax_total')::numeric=1000);
    perform pg_temp.ok(stage||': 영향 없는 명시 면세 메뉴는 추가 이력·반영 대기 없음',
    ...
    perform pg_temp.ok(stage||': 상속 메뉴는 진행 영업에서만 반영 대기·이전 금액 유지',
      (public.last_entity_change(s,'recipe',inherited)->>'has_pending_change')::boolean=(stage in ('open','break'))
      and (public.recipe_tax_app_state(s,inherited)#>>'{quote,tax_total}')::numeric=case when stage in ('open','break') then 1000 else 0 end);
    p:=jsonb_set(p,'{categories}','[]');
    v:=public.save_store_tax_profile(s,p,(v->>'profile_id')::uuid,(v->>'revision')::integer);
    perform pg_temp.ok(stage||': 삭제된 과세 분류는 복구하지 않고 매장 기본 면세를 따름',
      not exists(select 1 from public.menu_tax_overrides where recipe_id=category_menu and tax_profile_id=(v->>'profile_id')::uuid)
      and (public.pending_recipe_tax_quote(category_menu)->>'tax_total')::numeric=0);
    ...
    perform pg_temp.ok(stage||': 종료 후 현재 면세·반영 대기 해제',
      (public.recipe_tax_app_state(s,inherited)#>>'{quote,tax_total}')::numeric=0
      and public.last_entity_change(s,'recipe',inherited)->>'has_pending_change'='false');
    set local role postgres;
  end loop;
end $test$;
