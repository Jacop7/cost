...
do $test$
declare stage text; u uuid; s uuid; r uuid; r2 uuid; bd uuid; d date; basis_month text; n bigint; trends bigint; stamp timestamptz; pending boolean; body jsonb;
begin
  foreach stage in array array['before_open','open','break','closed'] loop
    ...
    s:=(public.create_store('고정지출 합성 '||stage,'Asia/Seoul')->>'store_id')::uuid; d:=public.store_local_date(s); basis_month:=to_char(d-interval '1 month','YYYY-MM');
    ...
    perform public.save_fixed_costs(s,basis_month,100000,'[{"key":"rent","total":20000}]');
    set local role postgres;
    ...
    perform public.save_fixed_costs(s,basis_month,100000,'[{"key":"rent","total":10000},{"key":"labor","total":10000}]');
    ...
    select updated_at into stamp from public.fixed_costs_monthly f where store_id=s and f.month=basis_month;
    perform public.save_fixed_costs(s,basis_month,100000,'[{"key":"rent","total":10000},{"key":"labor","total":10000}]');
    perform pg_temp.ok(stage||': 같은 값 재저장은 메뉴 이력·수정 시각 유지',
      (select count(*)=n+1 from public.entity_change_events where entity_id=r)
      and (select updated_at=stamp from public.fixed_costs_monthly f where store_id=s and f.month=basis_month));
    perform public.save_fixed_costs(s,basis_month,100000,'[{"key":"labor","total":30000}]');
    ...
    perform public.save_fixed_costs(s,basis_month,100000,'[]');
    perform pg_temp.ok(stage||': 전체 삭제도 0으로 반영하고 자동 갱신 기록',
      public.fixed_cost_rate(s,to_char(d,'YYYY-MM'))=0 and (select count(*)=n+3 from public.entity_change_events where entity_id=r));
    trends:=(select count(*) from public.profit_trends where recipe_id=r);
    ...
    perform public.save_fixed_costs(s,to_char(d-interval '2 months','YYYY-MM'),100000,'[{"key":"labor","total":40000}]');
    perform pg_temp.ok(stage||': 과거월 정정은 해당 월 손익에 반영하고 현재 메뉴로 과거 추이 점을 합성하지 않음',
      (select fixed_cost=40000 and revenue=100000 from public.monthly_pl m where store_id=s and m.month=to_char(d-interval '2 months','YYYY-MM'))
      and (select count(*)=trends from public.profit_trends where recipe_id=r)
