...
[국가·통화·세금 국제 출시 기획안](./docs/국가-통화-세금-국제출시-기획안.md)에 있다. 확장 schema·감사·
SQL 권위 계산 몸통은 `0179`~`0181`에 마련돼 있지만 capability는 꺼져 있고 앱 전환 전이므로,
현재 사용자 계산·표시는 계속 `0090` 계약을 따른다.
...
4. 빈 DB에 전체 마이그레이션 적용 + DB 39개 스위트 + 2세션 경합 + 로케일·국제 관할 실측 대조
