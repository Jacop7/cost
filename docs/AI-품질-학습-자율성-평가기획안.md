...
이 문서는 `ACTIVE`다. `DEC-AI-STAGE-8-ACTIVATION-APPROVAL-027`에 따라 `AGENTS.md` 책임 목록에
등재됐으며, 역할·승인 권한은 팀 구성안, 지식 상태·관계는 온톨로지, 요청 수신·라우팅은
오케스트레이션, 경로·README·검사 배치는 디렉터리 기획안이 소유한다. 본 문서는 이 경계 안에서
평가·Learning·자율성 승격·강등 절차의 단일 권위다.
...
이 결정은 오케스트레이션 기획안 §8.2의 현재 누적 집합 외부 교차검수 유효 r1·r2와 사람 최종 승격
결정에서 확정한다.
