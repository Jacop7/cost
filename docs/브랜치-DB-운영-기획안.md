...
현재 `admin-acl.sh`에는 `fix|check`만 있고 두 모드 모두 감사 전에 `supabase_admin` 전환 가능 여부로
중단한다. 따라서 아래 대체 게이트는 **설계만 완료됐고 실행 도구는 아직 없다**. §3.2 P1-1의
`audit` 모드와 회귀시험이 구현되기 전에는 첫 원격 배포의 ACL 단계에서 중단한다.
...
현재 스키마의 연쇄 삭제 위험은 첫 실사용자 전 P0 차단 항목이다. 목표 데이터 모델과 보존 원칙은
