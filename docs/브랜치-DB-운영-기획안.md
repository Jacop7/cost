...
| [국가·통화·세금 국제 출시 기획안](./국가-통화-세금-국제출시-기획안.md) | 국제 출시 제품 계약은 무엇인가 | 출시국, 언어·통화 경계, 세금 UX·공식·이관 계약 |
...
1. 한국·미국·영국·호주·캐나다 출시를 `INTL-1` 한 개의 세로 기능으로 등록했다.
2. 현재 `9.0909…%` 유효 세율과 언어→통화 결합 계약을 새 법정 세율·국가 프로필로 옮길 때
   **스키마 확장→데이터 감사·이관→새 RPC→앱 전환→구 계약 정리** 순서를 강제했다.
3. 구 앱이 기존 `tax_items`를 덮어쓰지 않도록 호환 기간 또는 최소 앱 버전 차단을 구현 전
   필수 결정으로 추가했다.
4. 기존 migration을 수정하지 않고 새 migration과 전진 복구만 사용하며, 5개국 합성 매장으로
   스테이징 검산한 뒤 운영에 적용하도록 했다.

개정 3판의 단명 세로 브랜치, DB 변경 직렬화, 보호 게이트, 백업·ACL·복구 원칙은 그대로 유지한다.
...
`INTL-1`은 국가·통화·세금 계약을 앱·core·types·DB·시험·문서까지 한 번에 완결하는 세로 기능이다.
제품·계산 계약은 [국가·통화·세금 국제 출시 기획안](./국가-통화-세금-국제출시-기획안.md)을 따른다.
...
COSTKEEP_APPROVED_DEPLOY_SHA=<40자리-main-SHA> \
COSTKEEP_STAGING_PROJECT_REF=<20자리-project-ref> \
corepack pnpm db:deploy:staging:plan

# 위 계획을 검토한 뒤에만 추가한다.
COSTKEEP_DEPLOY_CONFIRM=APPLY:staging:<project-ref>:<40자리-main-SHA> \
...
outbox 생성·Queue enqueue·외부 요청이 0건이라는 사후조건을 둔다. 기존 `costkeep-close-due`,
`costkeep-apply-breaks`, `costkeep-purge-changes`는 이 신규 integration Cron 활성화 규칙의 대상이 아니다.
...
3. `docs/국가-통화-세금-국제출시-기획안.md` — 출시국·언어·통화·세금 제품 계약
4. `docs/작업큐.md` — 상태·의존성·완료 조건의 단일 출처로 갱신
5. `CLAUDE.md`·`AGENTS.md` — 탭·데이터·검증·짝 문서 참조
6. `README.md`·`apps/mobile/README.md` — 저장소·앱 시작, 검증, 배포 환경
7. `ARCHITECTURE.md` — 현재 원장·전파 이벤트·검산값
8. `apps/mobile/src/features/README.md` — 화면 인벤토리
9. `packages/db/README.md` — 마이그레이션·fresh DB·ACL·업그레이드 경로
