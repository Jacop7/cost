...
> 현재 동기화 ID: `DS-20260916-002`
> 문서 동기화 상태: `IN_PROGRESS`
...
### DS-20260916-002 · 발주 후보 카드 두 줄 배치

- 발주 세 탭이 공통 OrderCardHeading, 메뉴 카드와 같은 내부 여백과 공통 Badge sm을 사용한다. 사유·제목·화살표를 같은 줄로 묶고 현재 / 최소 재고와 현재 수량 / 최소 수량을 다음 줄에 표시한다.
- 다음 시작점: 변경 범위의 독립검수와 전체 증빙 결속은 미완료다.


### DS-20260916-001 · 최소재고 명칭 통일

- 제품 표시·접근성·정형 이력 표시를 통일했다. 수량·DB 식별자·과거 원문은 유지한다.
- 다음 시작점: 현재 판본의 전체 감사 결속·독립검수 미완료를 확인한다.


### DS-20260908-011 · PRT-307 첫 P3 배치 상한 확정

- 작업 성격: 제품 마이그레이션과 분리한 허용량 계약 조정. 첫 식재료 aligned 배치의 최대
  12개 화면만 임시 차이로 허용하고 emergency divergence는 계속 0건으로 유지한다.
- UI 변경: 없음. 적용본은 `DESIGN_SYNC` 주석만 변경한다.
- 완료 조건:
  - PRT307-P0-CONTRACT · `../../scripts/three-surface-p0-check.mjs`
  - PRT307-SYNC-CONTRACT · `../../scripts/three-surface-sync-check.mjs`
- 미완료·후속: 식재료 12개 화면을 `migrationPending`으로 등록한 뒤 실제 Expo 화면을
  프로토타입 레이아웃과 확정 토큰·컴포넌트 계약으로 마이그레이션한다.
- 다음 시작점: exact-SHA Opus 검수 → 식재료 12화면 제품 배치.

### DS-20260908-010 · PRT-306 P3 마이그레이션 마감일 현실화

- 작업 성격: P3 제품 배치와 분리한 마감 계약 조정. 연말까지 12화면 단위 다섯 배치와 배치별
  검수 여유를 두되 임시 차이를 무기한 허용하지 않는다.
- UI 변경: 없음. 적용본은 `DESIGN_SYNC` 주석만 변경한다.
- 전체 화면 전수 검수: PASS — render 185 target·design/i18n 182 target 재측정,
  기존 알려진 렌더 위반 5건과 i18n atRisk 396건만 재현되고 신규·악화 0.
- 완료 조건:
  - PRT306-P0-CONTRACT · `../../scripts/three-surface-p0-check.mjs`
  - PRT306-SYNC-CONTRACT · `../../scripts/three-surface-sync-check.mjs`
- 미완료·후속: 첫 식재료 배치 상한 0→12의 별도 커밋과 제품 12화면 마이그레이션.
- 다음 시작점: exact-SHA Opus 검수 → 상한 12 별도 재기준선 → 식재료 배치.
...
- 작업 성격: Text 측정기의 제품 범위 결속 수정 뒤 옛 앱 SHA에 남아 있던 iPhone 1×/2.143×
  원시 증거를 현재 앱 SHA에서 실제 재측정한다.
- UI 변경: 없음. 적용본은 `DESIGN_SYNC` 주석만 변경한다.
- 전체 화면 전수 검수: PASS — render 185 target·design/i18n 182 target 재측정,
  기존 알려진 렌더 위반과 i18n atRisk만 재현되고 신규·악화 0.
- 완료 조건:
  - PRT303-NATIVE-TEXT-1X · `native-text-scale-ios-1x.json`
  - PRT303-NATIVE-TEXT-2X · `native-text-scale-ios-2x.json`
  - PRT303-NATIVE-TEXT-CHECK · `../../scripts/native-text-scale-evidence-check.mjs`
- 미완료·후속: P3 첫 배치 상한의 별도 커밋과 식재료 12화면 실제 마이그레이션.
- 다음 시작점: clean checkout 자체검수 → exact-SHA Opus 재검수 → P3 상한 12 → 식재료 배치.
...
- ORD-01 비교 행의 최종 표기: 현재 / 최소 재고 → 866 / 1,000 g.

- ORD-01 입고 예정 본문은 재고 내역의 공통 글꼴과 좌우 정보 배치를 적용한다. 날짜가 앞이고 도착 상태가 뒤다.

- ORD-01 입고 완료 후속 결정: 입고 예정과 동일한 본문 행·baseline을 재사용하고 날짜는 실제 입고일의 M/D 입고다. 주문일을 입고일로 사용하지 않는다. 상세 계약은 current-spec의 발주 목록 절을 따른다.
