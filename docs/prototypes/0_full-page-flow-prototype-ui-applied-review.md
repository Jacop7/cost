...
- 실제 Expo 앱 수정: S2 색 역할과 S3a·S4 토큰/컴포넌트 계약을 단계별로 적용했다. 이 장부의
  ...
  별도로 소유한다.
- 2026-09-06 `DS-20260906-018` / `PRT-258`: P1c 페이블 독립검수 PASS의 Minor를 반영해
  프로토타입 z-index 14건·8단계와 시스템 기본 모션을 보존했다. UI 변경은 없고 앱 6건과
  프로토타입 14건의 대응표·해시 결속만 보강했다.
...
## DS-20260905-003 · PRT-210 W1 종결 · 잔여 146건 분류 · `text.link` 정정

- 대상: 앱 매핑표(규칙 47 → 51) · 검사기 결과 JSON · 값 매핑 기획서 §4.7·§7.3·§7.3.1·§7.3.2·**§8.2a**·§8.3 ·
  대비 조합표 · 감사 스크립트 3개(시계↔표본 계약) · 게이트 `ps1` · 7문서 header/장부 ·
  적용본 표식 1줄. **적용본 CSS·JS·마크업 무변경**
  검수 범위 **앱 선언 3,677건 · 렌더 계약 185건 · 디자인 축 182건 · 글로벌 4패스 × 182건**
- 기대값: 페이블이 "솔라 분류 몫" 으로 확인한 146건을 마저 갈라 승인 대기를 소유자 몫만
  남긴다. `text.link` 정정과 시계↔표본 계약을 함께 넣는다.
- 실제값: **승인 대기 192 → 46, 전부 소유자 몫.** 셋 다 "질문" 이 아니라 **관계 확인**이었다 —
  `11` 은 §4.7 이 "변별로 나눠 확인 필요" 라고 이미 열어 둔 자리였고(세로 78 · 가로 8),
  스크롤 상단 17자리는 **한 글자도 다르지 않은 같은 모양**이라 수렴할 것이 없고 없는 것이
  이름이었으며, 인라인 10 은 관계가 **둘**이었다(라벨↔셰브런 8 · 아이콘 묶음 2).
  `text.link` `#1465DB` 정정으로 **경계값 넷 → 둘 · 열린 조합 셋 → 0**.
- PC 검수: 1280×900 185건 · 넘침 0 · viewport 이탈 0 · 미처리 예외 0 · `console.error` 0 ·
  폰트 실패 0 · face 단언 실패 0 · 금지 굵기 0 · PASS
- 모바일 검수: 320×720 · CSS 200% · 글자 200% 각 185건 PASS. 글로벌 4패스 각 182건 PASS
- 측정 (전부 보존된 결과 JSON에서 인용):
  - 적용본 SHA `7ce8bd49…203fb383` · `DS-20260905-003` ·
    시계 `2026-09-04T09:00:00+09:00` · 표본 최신 `08/30` · 간격 **5일**
  - 앱 배정 `PROPOSAL_COMPLETE` · 3,677 = 2,078 + 0 + 155 + 1,398 + 46 + 0 · 미분류 0 ·
    다중 일치 946 · 이동 대차 여섯 통 일치
  - 렌더 `violationCount` **5** · `atRisk` **408행 = 408키** · 미매핑 **0** ·
    대비 실패 **2,453 / 6,974** — `PRT-208` 이후 무변경
  - 대비 게이트 `PASS`(24쌍 · 경계값 2 · 열린 0) · 문서 주장 대조 `PASS`(주장 5건)
- 음성 시험 (전부 통과):
  - 시계↔표본 — `--clock=2026-10-01` 이면 "가장 최근 표본 08/30 에서 32일 떨어져 있다" 로
    종료 코드 3.
  - 대비 — `PRT-209` 의 셋(표면 한 톤 · 열린 목록 삭제 · 조합 소멸)이 그대로 유효하다.
- 미검수: 없음
- 제약: **`W1` 은 닫혔다**(페이블 `CLOSED` · 조건 없음). 다음은 `S1` 이고, 페이블이 완료 조건
  둘을 지정했다 — "대비 게이트를 `verify.mjs` ③에 결속하고 `tokens.ts` 를 원천으로 읽는다"
  (`design-sync-check.ps1` 은 CI 어디에도 없는 로컬 Windows 전용이다) · "shadow 는 5역할
  선언까지, 인라인 0 은 감사 확장 뒤로". `rowMinHeight`·`controlVisualHeight` 대응표는
  **역할 판정을 먼저** 하고 낸다. 백업 유지 — `backup/pre-token-apply-DS-019`(`dbcde27`).
- 증거: `../디자인-토큰-3계층-값-매핑-기획서.md` · `full-page-flow-prototype-app-map-check.json` ·
  `-app-token-map.json` · `-axis-measure.mjs` · `-axis-at.json` · `-role-measure.mjs` ·
  `-role-at.json` · `-doc-claims.json` · `-doc-claims-check.json` ·
  `-contrast-contract.json` · `-contrast-gate.json` · `-render-audit.json` ·
  `-i18n-stress.json` · `-i18n-known.json` · `-design-audit.json` · `-contrast-fix.json` ·
  `-token-map-check.json` · `-atrisk-key-proof.json`
- 결과: PASS

## DS-20260905-002 · PRT-209 페이블 W1 종결 판정 반영 · 축 3건 정정 · 시계 고정

- 대상: 앱 매핑표(규칙 38 → 47) · 검사기 결과 JSON · 값 매핑 기획서 §7.3·§7.3.1·**§8.3**·§9 ·
  새 보존 스크립트 4개와 산출물 4개 · 게이트 `ps1`(봉인 목록 + 결속 2종) ·
  감사 스크립트 3개(시계 고정) · 7문서 header/장부 · 적용본 표식 1줄.
  **적용본 CSS·JS·마크업 무변경**
  검수 범위 **앱 선언 3,677건 · 렌더 계약 185건 · 디자인 축 182건 · 글로벌 4패스 × 182건**
- 기대값: 페이블의 W1 종결 판정(차단 2 · 조건 5)을 전부 반영하고, 승인 대기가
  결정문대로 줄어드는지 확인한다. 페이블 예상치는 178 이다.
- 실제값: **192.** 차이 14 는 전부 검수 조건이 새로 연 자리다 — 차트색 4(`tokens.ts` 에 없어
  primitive 에서 뺐다) · 안내 본문 5(결정 5-3 이 다루지 않은 세 번째 갈래) ·
  섹션 라벨 아래 3 · 범례 점 2(반경 실측이 어긋났다). **그리고 네 가지가 새로 드러났다** —
  ① 축 판정 3건이 여전히 틀린 방향에 있었다(`Button.tsx:56` 은 앱의 모든 버튼이다).
  ② 반경 18건 중 두 자리가 원이 아니다. ③ 결정 2-4 의 `#1470F5` 가 `text.link` 로서
  흰 표면 밖에서 미달이다(4.31 · 4.08 · 4.03). ④ `rowMinHeight` 60/76/92 와
  `controlVisualHeight` 32/38 이 앱 실측과 맞지 않는다. ①②는 고쳤고 ③④는 §8.3 에 열었다.
  **⑤ 측정 재현성 결함.** 적용본이 한 글자도 안 바뀌었는데 디자인 감사 수치가 달라졌다 —
  프로토타입이 스스로 `new Date()` 를 읽어 표본 행이 7일을 넘으면 `recent-change` 버튼을
  통째로 지운다(`typo 6,974 → 6,965` 등). **봉인은 파일을 묶지 시계를 묶지 않는다.**
  감사 3종에 시계 고정을 넣어 재측정하니 PRT-208 수치와 완전히 일치했다.
- PC 검수: 1280×900 185건 · 넘침 0 · viewport 이탈 0 · 미처리 예외 0 · `console.error` 0 ·
  폰트 실패 0 · face 단언 실패 0 · 금지 굵기 0 · PASS
- 모바일 검수: 320×720 · CSS 200% · 글자 200% 각 185건 PASS. 글로벌 4패스 각 182건 PASS
- 측정 (전부 보존된 결과 JSON에서 인용):
  - 적용본 SHA `87d8aa6e…f3706a07` · `DS-20260905-002` · **시계 `2026-09-04T09:00:00+09:00`**
  - 앱 배정 `PROPOSAL_COMPLETE` · 3,677 = 2,078 + 0 + 153 + 1,254 + 192 + 0 · 미분류 0 ·
    다중 일치 1,704 · 이동 대차 여섯 통 일치(`487 − 304 + 9 = 192`)
  - 렌더 `violationCount` **5** · `atRisk` **408행 = 408키** · 미매핑 **0** ·
    대비 실패 **2,453 / 6,974** — 전부 `PRT-208` 과 동일
  - 새 게이트 — 문서 주장 대조 `PASS`(주장 5건) · 색 대비 `OPEN`(21쌍 · 경계값 4 · 열린 3)
- 음성 시험 (전부 통과):
  - 문서 주장 — `PRT-206` 이전 스크롤 목록을 되돌리면 `문서에만 있는 48·64·80` 과
    `코드에만 있는 28·30·96` 을 **둘 다** 잡는다. 굵기 400 을 되돌리면 유령 값으로 잡는다.
  - 색 대비 — 표면 `bg` 를 한 톤 어둡게 하면 그 자리에서 FAIL · 열린 목록에서 한 줄을 몰래
    빼면 FAIL · 열린 조합이 사라져도 사유 없이는 FAIL.
  - 시계 — `--clock=2026-10-01` 로 옮기면 6,965 로 떨어지고, 값이 잘못되면 종료 코드 2.
- 미검수: 없음
- 제약: **`W1` 은 아직 안 닫혔다** — 이 반영에 대한 페이블 재판정이 종결 조건이다.
  §8.3 의 넷 중 ③④ 는 소유자 확인이 필요하고, ②③ 은 `S3b` · `S2` 착수 전에 답이 있어야 한다.
  승인 대기 잔여 **146건**(출처 없는 수렴 119 · 스크롤 상단 17 · 인라인 10)은
  솔라 분류 몫이고 `S1` 과 병행한다. 백업 유지 — `backup/pre-token-apply-DS-019`(`dbcde27`).
- 증거: `../디자인-토큰-3계층-값-매핑-기획서.md` · `full-page-flow-prototype-app-map-check.json` ·
  `-app-token-map.json` · `-axis-measure.mjs` · `-axis-at.json` · `-role-measure.mjs` ·
  `-role-at.json` · `-doc-claims.json` · `-doc-claims-check.json` ·
  `-contrast-contract.json` · `-contrast-gate.json` · `-render-audit.json` ·
...
## DS-20260905-001 · PRT-208 소유자 결정 열 건 · 스크롤 값 목록 정정

- 대상: 값 매핑 기획서 §7.3.1·§8.2, 감사 결과 JSON, 7문서 header/장부, 적용본 표식 1줄.
  **적용본 CSS·JS·마크업 무변경**
  검수 범위 **앱 선언 3,677건 · 렌더 계약 185건 · 디자인 축 182건 · 글로벌 4패스 × 182건**
- 기대값: 소유자 결정을 정본에 기록하고, 인용된 수치가 실제 산출물과 맞는지 독립 검산한다.
- 실제값: **대비값 10개·건수 전부 일치.** 시안집은 다른 세션이 만들었으므로 다시 쟀고,
  소수점 둘째 자리까지 같았다. **다만 두 가지가 새로 나왔다** —
  ① 승인된 값 중 **셋이 AA 기준선에 여유 0**으로 붙어 있다(4.50 · 4.50 · 4.51).
  ② 기획서 §7.3.1 의 스크롤 값 목록에 **존재하지 않는 값 셋**(48·64·80)이 적혀 있었다.
  값 개수와 총 건수가 우연히 같아 지금까지 안 걸렸다.
- PC 검수: 1280×900 185건 · 넘침 0 · viewport 이탈 0 · 미처리 예외 0 · `console.error` 0 ·
  폰트 실패 0 · face 단언 실패 0 · 금지 굵기 0 · PASS
- 모바일 검수: 320×720 · CSS 200% · 글자 200% 각 185건 PASS. 글로벌 4패스 각 182건 PASS
- 측정 (전부 보존된 결과 JSON에서 인용):
  - 적용본 SHA `26676bfe…be5ad918` · `DS-20260905-001`
  - 앱 배정 `PROPOSAL_COMPLETE` · 3,677 = 2,082 + 0 + 156 + 952 + 487 + 0 · 미분류 0
  - 렌더 `violationCount` **5** · `atRisk` **408행 = 408키** · 미매핑 **0** ·
    대비 실패 **2,453 / 6,974**
- 미검수: 없음
- 제약: **`W1` 은 아직 안 닫혔다** — 페이블의 규칙 38개 재검수가 종결 조건이고,
  그 검수 요청이 아직 전달되지 않았다(데스크톱 앱 채팅 전환 문제).
  승인 대기 잔여 **146건**(출처 없는 수렴 119 · 스크롤 상단 17 · 인라인 10)은
  소유자 결정 대상이 아니라 솔라가 분류를 마저 해야 하는 것이다.
  `S1` 은 `W1` 종결 후 착수한다.
  백업 유지 — `backup/pre-token-apply-DS-019`(`dbcde27`).
- 증거: `../디자인-토큰-3계층-값-매핑-기획서.md` · `full-page-flow-prototype-app-map-check.json` ·
  `-app-token-map.json` · `-render-audit.json` · `-i18n-stress.json` · `-i18n-known.json` ·
  `-design-audit.json` · `-contrast-fix.json` · `-token-map-check.json` · `-atrisk-key-proof.json`
- 결과: PASS

## DS-20260904-028 · PRT-207 축 판정 정정 · 통 이동 대차

- 대상: 앱 매핑표 축 판정, 검사기(통 이동 대차), 동기화 검사 스크립트,
  값 매핑 기획서 §4.10·§7.3, 감사 결과 JSON, 7문서 header/장부, 적용본 표식 1줄.
  **적용본 CSS·JS·마크업 무변경**
  검수 범위 **앱 선언 3,677건 · 렌더 계약 185건 · 디자인 축 182건 · 글로벌 4패스 × 182건**
- 기대값: 늘어나는 간격의 방향(S3a/S3b)이 값이 아니라 축으로 판정돼야 하고,
  회차 간 통 이동이 도구가 낸 대차로 검산돼야 한다.
- 실제값: **축 판정이 54건 틀려 있었다.** `3`=세로 / `6`·`7`=가로 로 값만 보고 갈랐는데
  실제로는 `3` 45세로·5가로, `6` 27세로·74가로, `7` 22세로·27가로였다.
  그중 **5건은 가로인데 `S3a`(안전)에 있었다** — `S4` 보상 없이 폭을 늘릴 뻔했다.
  속성+컨테이너 방향으로 다시 갈라 `GROW-V` 94 · `GROW-H` 106.
  **통 이동 대차는 정확히 맞았고**, `PRT-206` 서술에서 행방이 빠졌던 48건이 드러났다.
  그 서술은 `파일:줄` 목록 길이를 선언 수로 적은 것이 원인이었다(조작 상자 43→**82** 등).
- PC 검수: 1280×900 185건 · 넘침 0 · viewport 이탈 0 · 미처리 예외 0 · `console.error` 0 ·
  폰트 실패 0 · face 단언 실패 0 · 금지 굵기 0 · PASS
- 모바일 검수: 320×720 · CSS 200% · 글자 200% 각 185건 PASS. 글로벌 4패스 각 182건 PASS
- 측정 (전부 보존된 결과 JSON에서 인용):
  - 적용본 SHA `b253399d…0fe5b8ec` · `DS-20260904-028`
  - 앱 배정 `PROPOSAL_COMPLETE` · **3,677 = 2,082 + 0 + 156 + 952 + 487 + 0** ·
    미분류 0 · 승인 예외 0 · 다중 일치 1,810 · 규칙 38개 · **대차 6통 전부 검산 일치**
  - 렌더 `violationCount` **5** · `atRisk` **408행 = 408키** · 미매핑 **0** ·
    대비 실패 **2,453 / 6,974**
- 미검수: 없음
- 제약: 기획서 본문 토큰 리터럴을 `tokens.ts` 와 대조하는 게이트 검사가 다음 회차 첫 항목이다
  (페이블 제안 — §4.9·§6.4·§4.7 잔여 셋이 그 검사 하나로 잡혔을 것이다).
  규칙 38개 재검수, 소유자 봉투 A·B·C 도 남았다.
  백업 유지 — `backup/pre-token-apply-DS-019`(`dbcde27`).
- 증거: `full-page-flow-prototype-app-map-check.json` · `-app-token-map.json` ·
  `-app-map-check.mjs` · `../디자인-토큰-3계층-값-매핑-기획서.md` · `-render-audit.json` ·
  `-i18n-stress.json` · `-i18n-known.json` · `-design-audit.json` · `-contrast-fix.json` ·
  `-token-map-check.json` · `-atrisk-key-proof.json`
- 결과: PASS

## DS-20260904-027 · PRT-206 "기존 결정" 이 아니었던 것 · 문서 셋이 코드와 달랐다

- 대상: 앱 매핑표(규칙 24 → 38), 검사기, 값 매핑 기획서 §4.5·§4.7·§4.9·§6.4·§7.3·§9,
  감사 결과 JSON, 7문서 header/장부, 적용본 표식 1줄. **적용본 CSS·JS·마크업 무변경**
  검수 범위 **앱 선언 3,677건 · 렌더 계약 185건 · 디자인 축 182건 · 글로벌 4패스 × 182건**
- 기대값: `defect` 로 배정한 값은 전부 문서에 출처가 있어야 하고, 방향(S3a/S3b)이 있어야 한다.
- 실제값: **출처가 없는 값이 셋 있었다.** `11` 은 §4.7 이 "확인 필요" 로 열어 둔 값이고
  `18`·`22` 는 §4.7 에 없다 — 119건을 `pendingApproval` 로 옮겼다.
  검사기에 넣은 "출처 없는 기존 결정 주장" 검사가 `R-TY-SIZE-CONVERGE` 도 잡아냈다.
  **그리고 문서 셋이 코드와 달랐다** — §4.9 `radius.sm` 6→8 · §6.4 뱃지 계약 6→8 ·
  §4.7 결론 `space` 7단계→6단계. 셋 다 `PRT-196` 이 매핑표만 고치고 본문을 안 고친 잔여다.
- PC 검수: 1280×900 185건 · 넘침 0 · viewport 이탈 0 · 미처리 예외 0 · `console.error` 0 ·
  폰트 실패 0 · face 단언 실패 0 · 금지 굵기 0 · PASS
- 모바일 검수: 320×720 · CSS 200% · 글자 200% 각 185건 PASS. 글로벌 4패스 각 182건 PASS
- 측정 (전부 보존된 결과 JSON에서 인용):
  - 적용본 SHA `8c4e9cbd…f02d34eb` · `DS-20260904-027`
  - 앱 배정 `PROPOSAL_COMPLETE` · **3,677 = 2,082 + 0 + 156 + 952 + 487 + 0** ·
    미분류 0 · 승인 예외 0 · 다중 일치 1,810 · 규칙 38개 전부 적중
  - 렌더 `violationCount` **5** · `atRisk` **408행 = 408키** · 미매핑 **0** ·
    대비 실패 **2,453 / 6,974**
- 미검수: 없음
- 제약: 페이블의 **규칙 38개 재검수**가 다음 회차 입력이다. 소유자 봉투 A(정본 확장) ·
  B(디자인 판단) · C(역할 배정 표) 미작성.
  백업 유지 — `backup/pre-token-apply-DS-019`(`dbcde27`).
- 증거: `full-page-flow-prototype-app-map-check.json` · `-app-token-map.json` ·
  `-app-map-check.mjs` · `../디자인-토큰-3계층-값-매핑-기획서.md` · `-render-audit.json` ·
  `-i18n-stress.json` · `-i18n-known.json` · `-design-audit.json` · `-contrast-fix.json` ·
...
## DS-20260904-026 · PRT-205 규칙 순서가 배정을 바꾼다

- 대상: 앱 매핑표·검사기, 동기화 검사 스크립트, 값 매핑 기획서 §7.3·§9, 감사 결과 JSON,
  7문서 header/장부, 적용본 표식 1줄. **적용본 CSS·JS·마크업 무변경**
  검수 범위 **앱 선언 3,677건 · 렌더 계약 185건 · 디자인 축 182건 · 글로벌 4패스 × 182건**
- 기대값: 규칙 순서가 배정에 영향을 주는지 세고, 영향을 준다면 그 순서가 규칙 자체에
  근거로 적혀 있어야 한다.
- 실제값: **1,009건이 순서에 걸려 있었다.** 일곱 쌍이 나왔고 가장 큰 것이 718건이다.
  `R-TY-SIZE-PRIM` 과 `R-TY-SIZE-OFFSCALE` 의 순서를 바꾸니 718건이 `primitive` 에서
  `pendingApproval` 로 옮겨 갔다. 일곱 쌍 전부에 순서근거를 썼고, 검사기가 이제
  순서근거 없는 순서 의존을 FAIL 시킨다.
  `marginTop:2` 5건(View 줄)은 `defect` 에서 `pendingApproval` 로 옮겼다 —
  안내 아이콘+문구라면 목표값이 4 가 아니라 8 이다.
- PC 검수: 1280×900 185건 · 넘침 0 · viewport 이탈 0 · 미처리 예외 0 · `console.error` 0 ·
  폰트 실패 0 · face 단언 실패 0 · 금지 굵기 0 · PASS
- 모바일 검수: 320×720 · CSS 200% · 글자 200% 각 185건 PASS. 글로벌 4패스 각 182건 PASS
- 측정 (전부 보존된 결과 JSON에서 인용):
  - 적용본 SHA `62f467fb…1126f9ef` · `DS-20260904-026`
  - 앱 배정 `PROPOSAL_COMPLETE` · 3,677 = 2,082 + 0 + 152 + 1,044 + 399 + 0 ·
    미분류 0 · 다중 일치 1,009 · 규칙 24개 전부 적중
  - 렌더 `violationCount` **5** · `atRisk` **408행 = 408키** · 미매핑 **0** ·
    대비 실패 **2,453 / 6,974**
- 미검수: 없음
- 제약: 규칙 24개의 **근거 판정이 다음 회차로 넘어간다** — 페이블이 "규칙 표를 못 봤습니다.
  보지 않은 것을 '약한 게 없다' 고 말하지 않겠습니다" 며 보류했다. 이 회차의 산출물
  자체는 전수 검사를 마쳤고, 남은 것은 **다음 검수 회차의 입력**(ID · 술어 · 목표 통 ·
  근거 · 적중 수 표)이다. 소유자 봉투 A(정본 확장 231건) · B(디자인 판단 2건) ·
  C(역할 배정 표 137건)도 미작성이다.
  백업 유지 — `backup/pre-token-apply-DS-019`(`dbcde27`).
- 증거: `full-page-flow-prototype-app-map-check.json` · `-app-token-map.json` ·
  `-app-map-check.mjs` · `../디자인-토큰-3계층-값-매핑-기획서.md` · `-render-audit.json` ·
  `-i18n-stress.json` · `-i18n-known.json` · `-design-audit.json` · `-contrast-fix.json` ·
...
## DS-20260904-025 · PRT-204 `W1` 앱 선언 3,677건 전수 배정

- 대상: 앱 매핑표·검사기 신설, 동기화 검사 스크립트, 값 매핑 기획서 §7·§7.3,
  감사 결과 JSON, 7문서 header/장부, 적용본 표식 1줄. **적용본 CSS·JS·마크업 무변경**
  검수 범위 **앱 선언 3,677건 · 렌더 계약 185건 · 디자인 축 182건 · 글로벌 4패스 × 182건**
- 기대값: 앱 선언 3,677건이 여섯 통 중 정확히 하나에 들어가고, 미분류와 승인 예외가 0 이어야 한다.
- 실제값: **3,677 = 2,082 + 0 + 152 + 1,049 + 394 + 0. 미분류 0 · 승인 예외 0.**
  `defect` 1,049 중 976건은 프로토타입이 이미 정한 수렴의 적용이고, 73건이 새 판정이다 —
  `marginTop:2`(제목·값 아래 보조 줄)를 `space.xs`(4) 결함으로 봤다.
  처음엔 프로토타입처럼 광학 보정으로 넣으려다 소스를 열어 관계를 확인하고 바꿨다.
  `semantic` 0 은 앱 감사가 값만 기록하고 역할을 기록하지 않기 때문이며, 지금 단계의 정확한 상태다.
- PC 검수: 1280×900 185건 · 넘침 0 · viewport 이탈 0 · 미처리 예외 0 · `console.error` 0 ·
  폰트 실패 0 · face 단언 실패 0 · 금지 굵기 0 · PASS
- 모바일 검수: 320×720 · CSS 200% · 글자 200% 각 185건 PASS. 글로벌 4패스 각 182건 PASS
- 측정 (전부 보존된 결과 JSON에서 인용):
  - 적용본 SHA `5faff462…50e381626` · `DS-20260904-025`
  - 앱 배정 `PROPOSAL_COMPLETE` · 선언 3,677 · 미분류 0 · 승인 예외 0 · 규칙 23개 전부 적중
  - 렌더 `violationCount` **5** · `atRisk` **408행 = 408키** · 미매핑 **0** ·
    대비 실패 **2,453 / 6,974**
- 미검수: 없음
- 제약: `pendingApproval` **394건**이 사람 결정 대기다 — `D-11` 정본 확장 167 ·
  `fontSize:15` 역할 배정 67 · RN `lineHeight` 62 · 스크롤 하단 여유 49 ·
  범주 타일 색 22 · 스케일 밖 글자 11 · 빈 상태 여백 10 · 팔레트 인접 색 4 · 차트 색 2.
  결정 전에는 `S1` 로 넘어가지 않는다.
  백업 유지 — `backup/pre-token-apply-DS-019`(`dbcde27`).
- 증거: `full-page-flow-prototype-app-map-check.json` · `-app-token-map.json` ·
  `-app-map-check.mjs` · `../token-adoption-audit.json` · `-render-audit.json` ·
  `-i18n-stress.json` · `-i18n-known.json` · `-design-audit.json` · `-contrast-fix.json` ·
  `-token-map-check.json` · `-atrisk-key-proof.json`
- 결과: PASS

## DS-20260904-024 · PRT-203 봉인이 증거를 요구한다

- 대상: 동기화 검사 스크립트, 맥락 장부 형식, 값 매핑 기획서 §7.2·§9, 감사 결과 JSON,
  7문서 header/장부, 적용본 표식 1줄. **적용본 CSS·JS·마크업 무변경**
  검수 범위 **렌더 계약 185건 · 디자인 축 182건 · 글로벌 4패스 × 182건 · 대비 전 텍스트**
- 기대값: 완료 조건마다 증거 경로가 없으면 봉인이 거부되어야 한다.
- 실제값: **세 경로 모두 거부됐다.** 빈 경로 · 없는 파일 · 봉인 밖 경로에서 각각 FAIL.
  (다) 검사를 넣으면서 **게이트 스크립트 자신과 값 매핑 기획서가 봉인 밖** 이라는 것이
  드러나 둘 다 봉인 대상에 넣었다. 그전에는 완료 후에 고쳐도 아무 경고가 없었다.
- PC 검수: 1280×900 185건 · 넘침 0 · viewport 이탈 0 · 미처리 예외 0 · `console.error` 0 ·
  폰트 실패 0 · face 단언 실패 0 · 금지 굵기 0 · PASS
- 모바일 검수: 320×720 · CSS 200% · 글자 200% 각 185건 PASS. 글로벌 4패스 각 182건 PASS
- 측정 (전부 보존된 결과 JSON에서 인용):
  - 적용본 SHA `4ff051fd…185a00ed` · `DS-20260904-024`
  - 렌더 `violationCount` **5** · 넘침 최대 **5px** · 이탈 최대 **76px**
  - `atRisk` **408행 = 408키** · 매핑 미매핑 **0** · 대비 실패 **2,453 / 6,974**
- 미검수: 없음
- 제약: `W1`(앱 선언 3,677건 매핑)이 다음이다. 구조화된 조건 목록 형식은 `W1` 뒤로 미뤘다.
  백업 유지 — `backup/pre-token-apply-DS-019`(`dbcde27`).
- 증거: `full-page-flow-prototype-design-sync-check.ps1` ·
  `../디자인-토큰-3계층-값-매핑-기획서.md` · `full-page-flow-prototype-atrisk-key-proof.json` ·
  `-render-audit.json` · `-i18n-stress.json` · `-i18n-known.json` · `-design-audit.json` ·
  `-contrast-fix.json` · `-token-map-check.json`
- 결과: PASS

## DS-20260904-023 · PRT-202 거짓 음성 재현 · 사라진 행 통과 금지

- 대상: 새 증명 스크립트·결과, 동기화 검사 스크립트, atRisk 기준선(`resolved` 신설),
  감사 결과 JSON, 7문서 header/장부, 적용본 표식 1줄. **적용본 CSS·JS·마크업 무변경**
  검수 범위 **렌더 계약 185건 · 디자인 축 182건 · 글로벌 4패스 × 182건 · 대비 전 텍스트**
- 기대값: 옛 키 스킴이 같은 악화를 **놓치는 것**이 실행으로 재현되어야 하고,
  `atRisk` 행이 사라지면 게이트가 통과시키지 않아야 한다.
- 실제값: **둘 다 확인됐다.** 원시 408행이 옛 키에서는 309칸으로 접혔다(충돌 99건 · 47묶음).
  `hostIndex` 46 의 36.9 → 41.9px 악화에 대해 옛 스킴 **PASS**, 새 스킴 **FAIL**.
  행 삭제는 현행 규칙에서 PASS, 새 규칙에서 FAIL 이었고, 새 규칙을 게이트에 넣었다.
  증명 스크립트 첫 판이 관측 쪽을 접지 않아 옛 스킴을 과대평가했던 것을 발견해 고쳤다.
- PC 검수: 1280×900 185건 · 넘침 0 · viewport 이탈 0 · 미처리 예외 0 · `console.error` 0 ·
  폰트 실패 0 · face 단언 실패 0 · 금지 굵기 0 · PASS
- 모바일 검수: 320×720 · CSS 200% · 글자 200% 각 185건 PASS. 글로벌 4패스 각 182건 PASS
- 측정 (전부 보존된 결과 JSON에서 인용):
  - 적용본 SHA `382f45ff…0f7667f0` · `DS-20260904-023`
  - `atRisk` 원시 **408행** · 옛 키 **309칸** · 새 키 **408칸** · 충돌 **99건**
  - 렌더 `violationCount` **5** · 넘침 최대 **5px** · 이탈 최대 **76px**
  - 매핑 미매핑 **0** · 대비 실패 **2,453 / 6,974**
- 미검수: 없음
- 제약: `-Finalize` 가 완료 조건마다 증거 파일 경로를 요구하게 만드는 일이 남았다.
  백업 유지 — `docs/prototypes/백업/0_full-page-flow-prototype-ui-applied_pre-PRT199.html` 과
  브랜치 `backup/pre-token-apply-DS-019`(`dbcde27`).
- 증거: `full-page-flow-prototype-atrisk-key-proof.json` · `-i18n-stress.json` ·
  `-i18n-known.json` · `-render-audit.json` · `-render-audit-known.json` ·
  `-design-audit.json` · `-contrast-fix.json` · `-token-map-check.json`
- 결과: PASS

## DS-20260904-022 · PRT-201 다섯 번째 음성 시험 — 겹치던 형제 host

- 대상: 변경 기록·검수 장부·맥락 장부, 감사 결과 JSON, 적용본 표식 1줄.
  **게이트 코드 무변경 · 적용본 CSS·JS·마크업 무변경**
  검수 범위 **렌더 계약 185건 · 디자인 축 182건 · 글로벌 4패스 × 182건 · 대비 전 텍스트**
- 기대값: 옛 키에서 서로를 덮어쓰던 형제 host 중 **하나만** 악화시켜도 게이트가 FAIL 해야 한다.
- 실제값: **FAIL 했다.** `screen:analytics` 의 `sales-menu-sub` 6행
  (부족 폭 36.9 / 34.6 / 27.6 / 27.4 / 27.1 / 26.8px) 중 첫 행만 36.9 → 41.9px 로 올리자
  `atRisk 악화 — w130|screen:analytics|…|46 부족 폭 36.9 → 41.9` 가 떴다.
  옛 키였다면 여섯 행이 한 칸을 공유해 마지막 값 26.8 만 기준선에 남았고,
  36.9 행의 악화는 비교조차 되지 않았을 것이다. 복구 후 재실행 PASS.
- PC 검수: 1280×900 185건 · 넘침 0 · viewport 이탈 0 · 미처리 예외 0 · `console.error` 0 ·
  폰트 실패 0 · face 단언 실패 0 · 금지 굵기 0 · PASS
- 모바일 검수: 320×720 · CSS 200% · 글자 200% 각 185건 PASS. 글로벌 4패스 각 182건 PASS
- 측정 (전부 보존된 결과 JSON에서 인용):
  ...
  - 렌더 `violationCount` **5** · 넘침 최대 **5px** · 이탈 최대 **76px**
  - `atRisk` **408행 = 408키** · `w130` 106 · `w150` 302
  - 매핑 미매핑 **0** · 대비 실패 **2,453 / 6,974**
- 미검수: 없음
- 제약: `W1`(앱 선언 3,677건 매핑)이 실행 단계 앞에 남아 있다.
  백업 유지 — `docs/prototypes/백업/0_full-page-flow-prototype-ui-applied_pre-PRT199.html` 과
  브랜치 `backup/pre-token-apply-DS-019`(`dbcde27`). 원복 가능하다.
- 증거: `full-page-flow-prototype-i18n-stress.json` · `-i18n-known.json` ·
  `-render-audit.json` · `-render-audit-known.json` · `-design-audit.json` ·
  `-contrast-fix.json` · `-token-map-check.json`
- 결과: PASS

## DS-20260904-021 · PRT-200 게이트 보정 완결 — 예외의 '크기' 와 키 유일성

- 대상: 렌더 감사 스크립트·예외 목록, 동기화 검사 스크립트, 값 매핑 기획서, 감사 결과 JSON,
  atRisk 기준선, 7문서 header/장부, 적용본 표식 1줄. **적용본 CSS·JS·마크업 무변경**
  검수 범위 **렌더 계약 185건 · 디자인 축 182건 · 글로벌 4패스 × 182건 · 대비 전 텍스트**
- 기대값: 새로 넣은 게이트 검사가 실제로 위반을 잡아야 하고, 정상 상태에서는 잡지 않아야 한다.
- 실제값: **네 가지 변형에서 모두 FAIL 했고, 원상 복구 후에는 실패 0 이었다.**
  `KD-001` 상한 5→4 에서 "실측 5 px 이 상한 4 px 초과", `KD-002` 76→70 에서
  "실측 76 px 이 상한 70 px 초과", `w130` `atRisk` 행 복제에서 "키 중복" 과
  "원시 행 409 과 고유 키 408 불일치" 두 건, `maxPx` 삭제에서 "maxPx 가 없음".
  함께 `w150` `atRisk` 를 기획서에서 332 → **302** 로 고쳤다 — 332 는 '덜 늘어남' 이고
  `atRisk` 는 302 다. `w130` 이 106/106 이라 두 열이 우연히 같았던 것을 규칙으로 착각했다.
- PC 검수: 1280×900 185건 · 넘침 0 · viewport 이탈 0 · 미처리 예외 0 · `console.error` 0 ·
  폰트 실패 0 · face 단언 실패 0 · 금지 굵기 0 · PASS
- 모바일 검수: 320×720 · CSS 200% · 글자 200% 각 185건 PASS. 글로벌 4패스 각 182건 PASS
- 측정 (전부 보존된 결과 JSON에서 인용):
  - 적용본 SHA `f358ae52…c571c1bd` · `DS-20260904-021`
  - 렌더 `violationCount` **5** · `mobile320t2` 넘침 최대 **5px**(`KD-001` 상한과 일치) ·
    `mobile320z2` 이탈 최대 **76px**(`KD-002` 상한과 일치)
  - `atRisk` **408행 = 408키** · `w130` 106 · `w150` 302
  - 매핑 미매핑 **0** · 대비 실패 **2,453 / 6,974**
- 미검수: 없음
- 제약: 실행 단계 `S1`~`S4` 앞에 `W1`(앱 선언 3,677건 매핑)이 남아 있다.
  백업 유지 — `docs/prototypes/백업/0_full-page-flow-prototype-ui-applied_pre-PRT199.html` 과
  브랜치 `backup/pre-token-apply-DS-019`(`dbcde27`). 원복 가능하다.
- 증거: `full-page-flow-prototype-render-audit.json` · `-render-audit-known.json` ·
  `-i18n-stress.json` · `-i18n-known.json` · `-design-audit.json` · `-contrast-fix.json` ·
  `-token-map-check.json`
- 결과: PASS

## DS-20260904-020 · PRT-199 atRisk 기준선 키 충돌 정정

- 대상: 글로벌 스트레스 스크립트, atRisk 잔여 목록, 동기화 검사 스크립트, 감사 결과 JSON,
  7문서 header/장부, 적용본 표식 1줄. **적용본 CSS·JS·마크업 무변경**
  검수 범위 **렌더 계약 185건 · 디자인 축 182건 · 글로벌 4패스 × 182건 · 대비 전 텍스트**
- 기대값: atRisk 기준선의 키가 행마다 유일해야 한다.
- 실제값: **유일하지 않았다. 408행이 309키로 뭉개져 있었다(충돌 99건).**
  한 화면의 같은 구조 행 여섯 개가 한 키를 공유했다. 다섯 행이 나빠져도 마지막 하나만
  그대로면 게이트가 통과시키는 상태였다. `PRT-196` 에서 개수 비교를 버린 이유가
  키 설계에도 그대로 적용되는데 그걸 놓쳤다. host DOM 순서 색인을 붙여 고쳤다.
- PC 검수: 1280×900 185건 · 넘침 0 · viewport 이탈 0 · 미처리 예외 0 · `console.error` 0 ·
  폰트 실패 0 · face 단언 실패 0 · 금지 굵기 0 · PASS
- 모바일 검수: 320×720 · CSS 200% · 글자 200% 각 185건 PASS. 글로벌 4패스 각 182건
- 측정 (전부 보존된 결과 JSON에서 인용):
  - 적용본 SHA `b7f22e44…021d4dfc` · `DS-20260904-020`
  - **atRisk 408행 → 408키 · 충돌 0** (이전 309키)
  - 렌더 `violationCount` **5** · 매핑 미매핑 0 · 대비 실패 2,453 / 6,974
- 미검수: 없음
- 제약: 실행 단계 `S1`~`S4` 는 검수 승인 후 시작한다. 백업 완료 —
  `docs/prototypes/백업/0_full-page-flow-prototype-ui-applied_pre-PRT199.html` 과
  브랜치 `backup/pre-token-apply-DS-019`(`dbcde27`). 원복 가능하다.
- 증거: `full-page-flow-prototype-i18n-stress.json` · `-i18n-known.json` ·
  `-design-audit.json` · `-render-audit.json` · `-contrast-fix.json` · `-token-map-check.json`
- 결과: PASS
...
- 기대값: `T.ter` 신규 사용 0, `COLOR.text.tertiary` 의미 역할 경유, 렌더·기하 변화 0.
- 실제값: S2 색 사용 래칫 PASS, S3d exact PASS, W1 2,310건 배정 불변.
- PC 검수: 1280×900 렌더·390×844 디자인 감사 재실행 · PASS
- 모바일 검수: 320×720·CSS 200%·글자 200%·i18n 4패스 재실행 · PASS
- 미검수: 없음
- 결과: PASS
- 증거: S3d·S2·터치·W1·렌더·디자인·i18n·대비 산출물과 DS 상태.
- 별도 후속: 차트 인접 구간 접근성 Finding과 iOS 네이티브 증거.

## DS-20260906-022 · PRT-263 S3d 증거 결속 보정

- 대상: S3d 적용 뒤 터치 기준선 제품입력해시와 해시 지점 전수표 시험.
- 기대값: 제품 UI diff 0, 터치 결과 불변, 해시 지점 14개, W1 미결정·미분류 0.
- 실제값: 터치 미달·형제 중첩 0, 해시 지점 14개, W1 2,310건 전수 배정.
- PC 검수: 1280×900 렌더·390×844 디자인 감사 재실행 · PASS
- 모바일 검수: 320×720·CSS 200%·글자 200%·i18n 4패스 재실행 · PASS
- 미검수: 없음
- 결과: PASS
- 증거: 터치 기준선·해시 인벤토리 시험·W1·렌더·디자인·i18n·대비 산출물과 DS 상태.
- 별도 후속: 차트 인접 구간 접근성 Finding과 iOS 네이티브 증거.

## DS-20260906-021 · PRT-262 S3d 소유자 결정 적용·W1 재배정

- 대상: 제품 커밋 `6b965a6`, S3d exact 변경 파일 21개와 W1 재배정.
- 기대값: 사용자 승인 32건을 역할 토큰으로 치환하고 W1 미분류·미결정을 0으로 닫는다.
- 실제값: 선언 2,310 · primitive 2,061 · componentOwned 249 · defect/pending/exception 0.
- PC 검수: 1280×900 렌더·390×844 디자인 감사 재실행 · PASS
- 모바일 검수: 320×720·CSS 200%·글자 200%·i18n 4패스 재실행 · PASS
- 미검수: 없음
- 결과: PASS
- 증거: S3d exact diff·W1·렌더·디자인·i18n·대비·P1c 산출물과 DS 상태.
- 별도 후속: 차트 인접 회색 구간 대비 접근성 Finding과 iOS 네이티브 증거.

## DS-20260906-020 · PRT-260 P1c 최종 증거 봉인

- 대상: P1c와 W1 및 프로토타입 감사·대비 산출물 전체.
- 기대값: 생성형 명령을 전부 먼저 실행하고 마지막 봉인 뒤 파일 변화 0.
- 실제값: 앱 제품 diff 0, P1c 앱 6·프로토타입 14·미매핑 0, W1 2,345·미분류 0.
- PC 검수: 1280×900 렌더·390×844 디자인 감사 재실행 · PASS
- 모바일 검수: 320×720·CSS 200%·글자 200%·i18n 4패스 재실행 · PASS
- 미검수: 없음
- 결과: PASS
- 증거: P1c·W1·렌더·디자인·i18n·대비 산출물과 DS 상태.

## DS-20260906-019 · PRT-259 P1c·W1 증거 재봉인

- 대상: DS-018 이후 동일 입력으로 재생성된 W1 산출물과 P1c 보존 산출물.
- 기대값: 제품 UI diff 0, P1c 앱 6·프로토타입 14·미매핑 0, W1 사용처 2,345·미분류 0.
- 실제값: 기대값과 일치하고 W1 pending 32를 유지했다.
- PC 검수: 1280×900 렌더·390×844 디자인 감사 재실행 · PASS
- 모바일 검수: 320×720·CSS 200%·글자 200%·i18n 4패스 재실행 · PASS
- 미검수: 없음
- 결과: PASS
- 증거: P1c·W1 산출물과 렌더/디자인/i18n/대비 감사 4종.

## DS-20260906-018 · PRT-258 P1c 프로토타입 레이어·시스템 모션 증거 보강

- 대상: 앱 모션·레이어 감사기와 프로토타입 적용본의 CSS 레이어 선언.
- 기대값: 앱 6건·프로토타입 z-index 14건/8단계 전수 보존, 명시 프로토타입 모션 0건,
  시스템 기본 모션 별도 기록, 미매핑 0.
- 실제값: 앱 `fade 4 / slide 1 / zIndex 30 1`, 프로토타입
  `1·2·5·10·11·12·20·24` 14건, 시스템 모션 ActivityIndicator JSX 5·navigator 7·
  pressed opacity 1. 대응표와 입력·검사기 해시를 산출물에 보존했다.
- PC 검수: UI 변경 없음. 1280×900 렌더·390×844 디자인 감사 재실행 · PASS
- 모바일 검수: 320×720·CSS 200%·글자 200%·i18n 4패스 재실행. 렌더 위반 5건과
  i18n atRisk 396건은 알려진 목록과 정확히 일치하며 신규·악화 0 · PASS
- 미검수: 없음
- 결과: PASS
- 증거: `full-page-flow-prototype-motion-layer-audit.json` ·
  `../../scripts/design-token-motion-layer-audit.test.mjs` · 렌더/디자인/i18n/대비 감사 4종.
...
## DS-20260908-011 · PRT-307 첫 P3 배치 상한 확정

- 대상: P3 첫 식재료 aligned 배치의 migration backlog 상한과 P0·생성 backlog 결속.
- 기대값: 상한만 0→12로 독립 변경되고 backlog 항목 0·제품 변경 0·emergency 상한 0이 유지된다.
- 실제값: `migrationBacklogMax: 12`, `entries: []`, `emergencyDivergenceMax: 0`으로 재측정했다.
- PC 검수: PASS
...
- 모바일 검수: PASS — Android 1×/2×와 실제 3점 탭, iPhone 1×/2.143×와 실제 3점 탭,
  iOS 제품 Text 137쌍을 같은 제품 tree에서 재검증.
- 미검수: 없음
- 결과: PASS
- 증거: `native-touch-android-receipt.json` · `native-touch-ios-2x.json` ·
  `native-text-scale-ios-2x.json` · `full-page-flow-prototype-app-map-check.json` ·
  `../../scripts/design-token-s4-contract.json` ·
  `../../scripts/native-touch-runtime-evidence-check.mjs` ·
  `../../scripts/native-text-scale-evidence-check.mjs`.
...
### DS-20260913-001 · 메뉴 비용 카드 공통화 (PRT-308)

- 메뉴 등록·수정·판매가 시뮬레이션에 상세와 동일한 0/1/다항목 비용 카드 규칙을 적용한다. 등록·수정의 추가/사용량 수정은 유지하고 판매 손익은 읽기 전용 요약이다. 기본 펼침, 마지막 접기 상태 저장, 단일 항목의 소계·토글 생략.
- 현재 계약·로컬 검증·범위 한계는 `full-page-flow-prototype-current-spec.md`의 DS-20260913-001가 단일 출처다. 증거: `.codex/profit-cost-cards-20260913/`. 전체 디자인 게이트나 외부 독립검수 통과로 확대 해석하지 않는다.
...
- 고정 지출은 서버 매장 날짜의 월로 기존 편집/관리 화면에, 세금은 메뉴 스택의 `/recipes/tax?settings=1`에서 동일한 MY 세금 화면을 재사용한다. 뒤로가기로 메뉴 초안·시뮬레이션을 보존한다.
...
## DS-20260916-001 · 최소재고 명칭 통일

- 대상: 재료 목록·폼·상세, 발주, 알림, 재고 확인 및 이력 시스템 라벨.
- 기대값: 현재 제품 명칭 최소재고, 내부 식별자·사용자 입력 보존.
- 실제값: 현재 소스 문구와 정본 HTML 치환, 이력은 의미 키 기반 표시 변환.
- PC 검수: 실제 AppMap의 재료 목록·상세 표시 확인.
- 모바일 검수: 390px 웹 미리보기 표시 확인, 네이티브 별도 검수 미완료.
- 미검수: 전체 화면 재캡처·독립검수.
- 결과: 문구 반영, 전체 게이트 미완료.
- 증거: .codex/minimum-stock-20260916/inventory.json, mobile-tests.log, appmap-tests.log.

## DS-20260916-002 · 발주 후보 카드

- 대상: ORD-01 발주 후보 카드.
- 기대값: 사유·재료명·화살표 / 현재 재고·최소 뱃지·우측 수량 / 주문하기.
- 실제값: 실제 Expo 390px에서 최소 1kg·866g의 두 줄 배치 확인.
- PC 검수: 앱맵 실제 Expo에서 배치 확인.
- 모바일 검수: 웹 320px·390px 별도 확인, 네이티브 미검수.
- 미검수: 네이티브·전체 렌더 감사·독립검수.
- 결과: 제품 반영, 전체 게이트 미완료.
- 증거: .codex/order-candidate-style-20260916/orders-tests.log.

### 발주 후보 비교 행 후속 수정 (2026-09-16)

- 사용자 요청으로 최소 수량 뱃지를 제거하고 `현재 / 최소 재고`와 `현재 수량 / 최소 수량`을 배치했다.
- 실제 Expo 390px의 고춧가루는 저장된 값에 따라 `866g / 1,000g`으로 표시된다. 3,000g은 formatter 회귀시험에서 확인했다.
- 발주 27시험·core 270시험 통과(13개 건너뜀). 음수 현재 수량의 값과 위험 색상은 유지한다.

### 발주 후보 상단 여백 후속 수정 (2026-09-16)

- 메뉴 카드의 세로 12px·가로 16px을 적용하고 제목 단독 44px 높이를 제거했다. 제목·재고 묶음은 상세 진입 최소 44px을 유지하고 주문 버튼은 별도다.
- 뱃지는 두 화면 모두 공통 Badge sm·COMPONENT.badge를 사용하며 색상만 의미 역할에 따른다. 실제 Expo 390px 표시와 발주 27시험을 확인했다.

### 발주 세 탭 첫 행 통일 (2026-09-16)

- 입고 예정·입고 완료도 후보와 같은 상태 뱃지·재료명·화살표 배치와 여백을 적용했다. 실제 Expo의 세 탭을 확인했다.
- 공통 OrderCardHeading을 사용하며 지연·부분입고·발주함은 기존 조건을 유지한다. 카드 본문까지 상세 진입 영역에 포함하고 별도 입고·취소 버튼은 유지한다.
- 발주 시험 27/27 통과. 기존 전체 증빙 결속·독립검수 제한은 유지한다.

- 2026-09-16 ORD-01 단위 표기 후속: 866 / 1,000 g 형식, 발주 시험 27/27 통과.

- 2026-09-16 입고 예정: 실제 Expo 320px·390px에서 9/17 내일 도착·마장축산·13.00원/g · 130,000원 및 우측 총 10kg·(5kg × 2개) 확인. 발주 시험 27/27 통과, 부분입고도 전체 주문량·총액 기준임을 검증.

- 2026-09-16 입고 예정 행 정렬: 구매처↔총량, 단가·총액↔포장 구성을 각각 같은 행과 baseline에 배치. 실제 Expo 확인 및 발주 27시험 통과.

- ORD-CARD-20260916 입고 완료 후속 검증: 실제 Expo/AppMap 320px·390px에서 같은 행 정렬, 입고 원장 날짜, 괄호 없는 포장 구성을 확인했다. mobile 타입 검사 및 발주 화면 27개·입고 날짜 4개 테스트 통과. 취소/재입고 회차, 페이지 경계 취소, 매장 범위, 조회 실패, 매장 시간대 날짜를 검증했다. 전체 verify·독립검수 완료를 의미하지 않으며 기존 제공자 한도 제한을 유지한다.
