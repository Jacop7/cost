...
- 이번 개정 제외: DB, RPC. 색 역할은 S2에서 Expo와 프로토타입에 함께 적용했고, Expo 구조·기하는
  S3a·S4의 명시된 토큰/컴포넌트 계약만 단계별 게이트 아래 적용한다.
...
값이며, 현재 일반-flow 탭에서는 본문 하단 여백에 다시 더하지 않는다. absolute/overlay 탭으로
바뀐 경우에만 `tabBarHeight + gap`을 사용한다.
...
- 카테고리 순서 이동은 Row 왼쪽의 단일 `44×44px` reorder 진입점이다. 누르면 가능한 방향
  (`위로 이동`·`아래로 이동`)을 네이티브 선택창에서 고른다. 옛 `28×20px` 위·아래 두 버튼은
  hitSlop이 겹쳐 독립 터치 영역을 만들 수 없으므로 사용하지 않는다. 항목이 하나뿐이면 진입점을
  비활성 처리한다.
...
- 서식 단일 출처는 `@costkeep/core`의 locale formatter다.
