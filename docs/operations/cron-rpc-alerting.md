...
1. 올바른 토큰 호출이 HTTP `200`, `status=ok`, `cron.monitored=true`, Cron 세 작업
   `healthy=true`인지 확인한다.
2. 토큰 없이 호출해 `401`인지 확인한다.
3. 스테이징에서 `margincook-close-due` 한 작업만 잠시 `active=false`로 바꾼다.
4. workflow를 수동 실행해 HTTP `503`과 `[ops-health] staging degraded` 이슈 생성을 확인한다.
...
- RPC fingerprint가 있으면 code·detail·플랫폼만 보고 동일 배포 SHA에서 재현한다. 원문 오류나 사용자
  입력을 이 원장에 추가하지 않는다.
