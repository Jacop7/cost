...
#### 2026-09-09 실행 효율 보완 — 공통 구조별 묶음

첫 적용 제품 `6931e55`: 판매 수량·기타 매출·지출 세 시트를 한 배치로 수정했다.
host11/11·type·전체 mobile605/605, 웹15조건/33장 수집 후 별도 Sol 세 target 한정 PASS.
상세는 `docs/ai-review/evidence/PROTOTYPE-EXPO-P3-SALES-REVIEW-20260909.md`의
「세 입력 시트 묶음」에 보존했다. 이 결과는 native/전체 P3 종결을 대신하지 않는다.

사용자 요청: 작업 속도를 개선하되 페이지·세부 상태 확인을 생략하지 않는다.
기존 개발/최종 종결 분리와 토큰 권위·공식 게이트는 유지한다.

- 수정 단위를 같은 공통 구조·같은 위험 경계를 가진 소비처 묶음으로 잡는다. 첫 묶음은
  SALES-05b/06/07(판매 수량·기타 매출·지출) 세 입력 시트다. 서로 다른 저장 정책은 통합하지 않는다.
- 편집 반복에서는 관련 실제 host 시험과 타입검사를 사용한다. 배치의 안정된 후보에서 전체
  모바일 회귀·전후 캡처·Sol 내부검수를 묶는다. 공식 최종 verify6/6 조건을 낮추지 않는다.
- 동일 제품·수집기·입력 범위의 기존 before는 blob/입력 근거를 확인해 재사용한다. 검수 이력이
  없는 다른 host나 다른 상태로 표본을 전파하지 않는다. after는 현재 후보에서 직접 수집한다.
- 검수 패킷은 exact diff·대상 목록·전후 증거·잔여에 한정한다. 새 Finding의 영향 범위만
  수정/재검수하고 독립적인 다음 배치의 읽기·준비를 병행한다. 공용 고위험은 별도 경계로 둔다.
- 새 검사기/보존 문서는 필요한 증거가 없을 때만 만든다. 배치별 실행서 짧은 기록과 기존
  도메인 검수 장부를 사용한다. 작업·측정·검수 시간을 관측하기 전 배속이나 완료시간을 약속하지 않는다.
- 185 target 장부의 각 행은 별도로 유지한다. 묶음 PASS는 그 묶음의 명시된 상태만 갱신하며
  `표본 내부검수`를 `페이지 전체 완료`로 바꾸지 않는다.
...
- 한 배치는 소스 분류→공용 기준 구현→자체검수→Sol 페이지 검수→차단 수정으로 전진한다.
  공용 계약 변경·Sol 미해결 반복 실패는 필요 시 Astra 검수를 추가한다. 도메인 마감만으로
  중복 호출하지 않는다(사용자 승인2026-09-09). 새로운 검사기는 다음 배치에 필요한
...
**메뉴 첫 배치(2026-09-09):** `90a1284`에서 RCP-01의3필터를 기존 FilterButton, 정렬을
SortSheet로 통일했다. 목록 제목/금액 wrap·검색 위치·헤더 검색 닫기 초기화도 적용했다.
메뉴 전체13표면/49target 중 이번 범위는 목록+3popup의4target이며 최종 완료4건이라는 뜻은 아니다.
내부 Sol18시험·Astra 제한 범위 PASS, 전체 모바일474/474. 전후15+15+21PNG와 의도된
공용 폰트 차이·매출 공용 소비처 재측정·FAB 가림 잔여는
`docs/ai-review/evidence/PROTOTYPE-EXPO-P3-RECIPES-REVIEW-20260909.md`에 기록했다.
후속 `0a3912f`/`ffd4541`에서 상세·등록·수정의 공용 Button/ScrollTabs 채택과 카테고리
접근성 정정을, `156018c`에서 RCP-10/11 검색 이름·상세 판매가 범례 wrap을 진행했다.
내부 한정 검수와 모바일508/508은 PASS, 상세 환경404·차트 확대·전체스크롤·공식검수 잔여는
같은 검수 장부에 보존한다. 완료 target 수로 환산하지 않는다.
후속 `34f3cdf` 손익 변동 공용행/시트와 `e2f8eb7` 부자재·카테고리 배치는 내부 한정 검수 PASS.
부자재 정상390 두 칸 회귀와 지연 저장 콜백의 새 폼 오염은 수정·재검수했다. 최신 모바일
55파일564/564·타입 PASS이며 실제 RPC 쓰기/네이티브/공식 외부 검수 증거는 아니다.
다음은 시뮬레이션·평균판매량 등 메뉴 잔여 확인 후 발주 배치다. 전후·기각 후보·수집 clean
실패 원본과 범위 한계는 같은 검수 장부에 계속 보존한다.
사용자 재확인으로 PRT-131 월평균 제거 누락을 발견했다. `e51a1fc`에서 입력·상세 행·탭·
legacy 월평균 정렬을 제거하고 옛 주소 redirect/카탈로그 제외를 반영했다. 기존 DB 값은
저장 키 생략으로 유지한다. `378bef4`는 별도 시뮬레이션 wrap 수정이다. 내부 관련35시험/
Astra한정 PASS, 전체모바일568/568·타입 PASS. 45PNG/오류18·쓰기차단9의 환경 한계와
사용자 정정 경위는 검수 장부에 보존했다. RCP07 호환 ID는 활성 기능/완료 페이지가 아니다.
...
내부 검수 분담(사용자 승인2026-09-09, 전판2026-09-08 대체): 일반 페이지마다 자체검수 뒤
별도 Sol high 읽기 전용 검수·수정·재검수 PASS를 요구한다. Astra 상시 검수는 중단하고 공용
컴포넌트·토큰·접근성 계약 변경 또는 Sol 미해결 반복 실패에만 필요 근거를 남겨 추가한다.
도메인 마감만으로 Astra를 중복 호출하지 않는다. 입력은 변경 diff·관련 계약·전후 캡처·시험으로
한정하며 전체 대화 재독을 피한다. 동일한 전후 캡처·확대·회귀시험·exact SHA 조건을
...
| ING-02 | Field/Input/Select/Button, 거래처·최소재고 등 실제 필드 | 공용 Input 기본 크기/굵기/패딩 불변. 프로토타입 필드 수로 축소하지 않음 | 등록 폼·단위 선택 외형 |
...
#### 발주 도메인 진행 — 2026-09-09

ORD-01 목록의 공용 ScrollTabs/전체 건수·긴 이름·검색 닫기(ff626f0), 주문/입고 시트의
긴 본문·좁은 입력·1:1 동등 높이 행동(4f6b220)은 Sol 한정PASS다. ScrollTabs optional count
