# AI-ORCH-PLANS-BUNDLE-A-FABLE-SOFTCAP-003 공동 작업 장부

> 1~5단계 다섯 공식 기획안의 구조 폐쇄성을 Fable이 읽기 전용으로 독립 검수한다.
> 두 선행 실패 Task는 판정 없이 예산 소진됐으며 이 장부는 그 결과를 PASS로 승계하지 않는다.
...
- 이번에 바꾼 내용: Sol high의 6건 Finding을 반영한 DRAFT_READY 바이트를 유지하고, 두 선행 Fable 무판정 실패 원본을 별도 증거로 보존했다.
...
- 실행한 테스트·현재 증거: Sol high 재검수 PASS, `corepack pnpm ai:plans:simulate` 71/71 PASS, 두 선행 Fable 실패 원본 보존, hard-cap guard self-test 51/51 PASS
- 사람 결정이 필요한 항목: 네 DRAFT의 ACTIVE 승격은 8단계에서 별도 결정한다.
- 검수 예산: `DEC-AI-REVIEW-BUDGET-A-SOFTCAP-003`의 r001 soft cap USD 4.00. 실제 청구 초과 가능성 승인. 자동 재시도·재증액·Opus 전환 금지.
...
- 결정: 새 Fable 단일 검수 회차의 soft cap을 USD 4.00으로 승인하며 CLI 사후 정산액이 이를 초과할 수 있음을 수용한다.
- 허용 범위·기한: `AI-ORCH-PLANS-BUNDLE-A-FABLE-SOFTCAP-003/r001` 한 회차, 읽기 전용 검수, 2026-09-03 현재 작업 완료까지.
- 금지: 자동 재시도, 자동 증액, 동일 목적 Opus 동시·후속 우회, 다른 Task·회차로 승인 재사용.
- 근거: 사용자가 초과 가능성 안내 뒤 예산 금액으로 `4`를 명시했다.
