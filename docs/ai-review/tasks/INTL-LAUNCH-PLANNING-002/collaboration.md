# INTL-LAUNCH-PLANNING-002 공동 작업 장부
...
- setup_note: `INTL-LAUNCH-PLANNING-001`은 외부 모델 호출 전에 target `AGENTS.md` 바이트 해시 불일치로 중단됐으며, 그 append-only 초기 기록을 보존하고 이 새 패킷에서 정확한 Git blob 해시를 사용한다.
