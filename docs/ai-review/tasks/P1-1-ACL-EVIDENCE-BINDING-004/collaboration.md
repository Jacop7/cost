# P1-1-ACL-EVIDENCE-BINDING-004 공동 작업 장부

> 실행 증거의 판본·해시·stderr 원문 결속만 독립적으로 검수한다.
...
- artifact_hashes: `[{ path: docs/ai-review/evidence/P1-1-ACL-SUPPORT-SOURCE-002.md, sha256: 39628aba94673aa5961181a266933bb6ac6a02a0f540edab68e8c98cb14b88f3, change_type: MODIFIED }]`
- 근거 Finding: `P1-1-SUPPORT-003-EVIDENCE-VERBATIM-BINDING`
- 반영 내용: V1/V2 commit·시험 SHA를 표로 고정하고, 모든 사보타주 행을 실행 판본과 실제 stderr에 연결했다.
- 검증: `git show <commit>:packages/db/scripts/admin-acl-audit.test.mjs` SHA-256이 증거의 V1·V2 값과 일치했다. 증거 문서만 변경했으므로 코드 재실행은 요구하지 않았다.
...
- finding_ids: `P1-1-BINDING-004-FINAL-VERIFY-V2-UNBOUND`, `P1-1-BINDING-004-V1-WORDING-NOTE`
- disposition: `APPLIED`
- 적용 내용: 최종 verify 6/6·fresh DB 0개·보안 관측값이 V2 commit `474d087f`와 시험 SHA `380afade…a6575` 작업 트리에서 나온 값임을 최종 검증 절에 명시했다. V1 부채 문구가 정수 검사 이전 원문이고 V2에서 바뀌었다는 주석도 추가했다.
...
- finding_ids: `P1-1-BINDING-004-FINAL-VERIFY-V2-UNBOUND`
- 판본 대조: V2 commit과 시험 SHA는 직전 `git show` 실측값이며, 6/6 전체 검증은 V2 코드가 작업 트리에 적용된 뒤 실행됐다. 실행 뒤 `fresh_%` 0개와 0/32/87 관측값을 같은 흐름에서 확인했다.
- 변경 범위: 증거 문장만 수정했으므로 제품·시험 코드 재실행은 필요하지 않다.
