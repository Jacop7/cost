...
const cli = 'C:/Users/jacop/AppData/Local/OpenAI/Codex/bin/9ba750cce02d5e5c/codex.exe';
const output = new URL('./TEAM-SERVICE-MODEL-CATALOG-20260906-002.json', import.meta.url);
...
function finish(status, reason = null) {
  ...
  const record = {kind:'DESKTOP_BUNDLED_CLI_MODEL_LIST_OBSERVATION_NOT_PROVIDER_ATTESTATION', observed_at:new Date().toISOString(),
    ...
    scope:'Desktop-bundled CLI in a separate read-only app-server process, not the active desktop server or paid inference, and not host caller/receipt evidence',
  ...
  lines.close(); child.stdin.on('error', () => {}); child.stdin.end(); child.kill(); process.exitCode = status === 'COMPLETE' && matched ? 0 : 2;
}
...
lines.on('line', raw => {
  ...
  if (msg.id === 0) {
    ...
    send({method:'model/list',id:1,params:{limit:100,includeHidden:true}});
    ...
    if (msg.result.nextCursor) {
      ...
      send({method:'model/list',id:msg.id+1,params:{limit:100,includeHidden:true,cursor:msg.result.nextCursor}});
