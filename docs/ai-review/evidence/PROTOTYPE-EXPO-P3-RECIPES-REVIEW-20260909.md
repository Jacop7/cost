...
## 사용자 정정: 월평균 기능 제거 누락 (2026-09-09, 내부 한정 PASS)
...
`e51a1fc`로 UI 제거를 반영했다. Astra가 추가로 발견한 목록의 `판매량 많은순`도 legacy 월평균
값으로 정렬하던 실제 소비였다. 이를 제거해 나머지4종 정렬을 유지했다. 새로운 실제판매 집계나
RPC를 임의로 만들지 않았다. 관련 시험은 먼저 기존 제품에서 정렬보기 부재 RED1건을 확인한 뒤
수정 후 통과했다. RCP07은 registry ID만 보존하고 catalogMode=unsupported로 선택에서 제외한다.

Sol 갱신4파일18/18, root 전체 모바일57파일568/568·타입 PASS. Astra는 최종 e51a1fc/378bef4
diff와5파일35/35를 직접 확인해 내부 한정 PASS했다. 동기화 검사도 exact commit에서
64등록ID/53route/185prototype·orphan0 PASS다. 호환 ID를 제품 완료 페이지로 합산하지 않는다.

monthly-retirement-after-20260909(`378bef4`)는15조건45PNG이며 모든 해시 일치,
폰트/배율/문서 가로 넘침0. 옛 주소3조건은 실제 `/recipes/add`로 이동했고 입력 필드가 없다.
redirect 자체는 초안을 쓰지 않지만 목적지 추가 폼은 기존 정책대로 수정 초안을 초기화할 수
있으므로 이전 수정 draft 복원을 보장한다고 쓰지 않는다.
상세/시뮬레이션의 기존 app_capabilities404와 오류보고 쓰기 차단으로 errors18/blocked9,
수집기 exit1이다. 실 DB 보존 실행·네이티브·공식 검수·전체 verify PASS로 확대하지 않는다.

## 판매가 시뮬레이션 값 줄바꿈 (2026-09-09)

`378bef4`: 고정 라벨/값 열을 행별 wrap으로 바꾸고, 손익 구성의46px 비율 폭을 제거했다.
기존 Card/Sheet/Slider/Badge/Button을 유지하며 계산·round/won·0원 opacity0.45는 변경0.
정상390은 기존 가로 배치를 유지하고320text2의21.2%/212원·배지·원가 비율 전문이 보인다.
초기 simulation-before-20260909는 동일 문구를 가진 배경 상세를 스크롤했으므로 시트 anchor
증거에서 제외한다. 수집기를 실제 dialog에 한정한 simulation-scoped-before-20260909
(`c88b23f`,6조건21PNG)를 before로 삼는다. 원본 둘 다 보존한다.
after는 위 monthly-retirement-after의시뮬레이션3조건12PNG 부분이다. recipe_detail 입력 해시
cf292273…는 전후 동일. Astra 정상390/확대 비율·금액·끝 닫기 직접 확인, 내부 시각 한정 PASS.
전체 스크롤/다른 슬라이더 가격 실렌더/모든 데이터/네이티브 완료는 아니다.
