...
현재 남은 조건: DB 타입 재생성, 업그레이드/전체verify6/6, 실제 화면/네이티브 검증, 정확한 현재 후보의 P3 범위·증거 승계와 정식독립검수, 동일SHA보호CI, 운영대상확인 및 배포가드/배포후점검. 영어번역은 제외한다. 앱배포대상확인을사용자에게질문했다. EAS에는development프로필만있으며DB배포영수증을앱운영배포증거로쓰지않는다.
