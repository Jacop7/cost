...
현재 상태: **2026-09-11 18:15 KST 기준 코드 main 반영·개발/운영 DB 배포·아래 사후 확인 완료**. 배포 SHA는 `7585b16dfbbc7d071c20b394c9c0a844d89a91d9`이다. 기기 증빙과 독립검수는 후속 상태이며, 새 앱스토어/Play Store/OTA 배포를 뜻하지 않는다.

## 최신 완료 — main 및 원격 DB 적용

- 브랜치 CI `34580002018`, main CI `34581508525`가 정확한 같은 SHA에서 성공했다. Node20/24 빠른 검사, 전체6단계, 업그레이드24/24, protected-gate 성공. 네이티브 기기 증빙의 별도 advisory 실패는 보존하며 접근성 완료로 표시하지 않는다.
- 사용자 승인 범위의 일반 fast-forward push로 main을 반영했다. 보호 규칙 bypass/force push는 하지 않았다. `deploy-20260911-7585b16` annotated tag도 origin에 푸시했다.
- 개발 `cvfvmpzcldyqurcrappu`: [APPLIED 영수증](../../deployments/2026-09-11T09-11-25-866Z-staging-7585b16.json). 운영 `smxaozdgoxbafjldoayb`: [APPLIED 영수증](../../deployments/2026-09-11T09-13-33-332Z-production-7585b16.json). 각각 guarded plan/apply로0192~0205의14개를 적용했고, 적용 후 pending0을 확인했다.
- 배포 전 양쪽 원격의 완료된 일일 물리 백업을 확인했다. 원격 `inventory_events`와 `inventory_states`는 모두 배포 전후0행·동일 해시이며 삭제/초기화하지 않았다. 로컬 개발 DB는 이번 원격 적용 명령의 대상이 아니다.
- 개발환경의 KR/US/GB/AU/CA 합성 검산 통과. 정확한 rollback marker와 전후 합성 계정 잔여0을 확인했다. 운영에 시험 데이터를 생성하지 않았다.
- 양쪽 앱 ACL 감사22개 지표 확인: 누락 facade0, 승인 facade80, 미승인 authenticated RPC0, 원장 직접쓰기0. 플랫폼 기본 ACL informational 항목은 그대로이며 `supabase_admin` 전환/셸 래퍼까지 검증했다고 주장하지 않는다.
- 양쪽에서 재고 수량 변경/취소 후보/취소, 판매가 시뮬레이션/초안 손익/권장 판매가6개 RPC 존재와0205 이력·국제 세금 capability를 확인했다. HTTP Auth health200, 비로그인 capability 호출401/42501은 기존0178 권한 계약대로 거부됨을 확인했다. 인증 사용자 화면의 저장 시나리오를 이 HTTP 검사로 대체했다고 주장하지 않는다.
- 운영 ACL 첫 조회는 결과 없이 transport 종료되어 `NOT_VERIFIED`로 보존했다. 활성 비유휴 DB 세션이 없음을 확인한 후 같은 롤백형 SQL을 별도 attempt2로1회 재시도해 통과했다. 최초 익명 HTTP200 기대는 점검 도구의 잘못된 가정이었으며, 실제401과 원 권한 계약을 보존했다. 제품 권한을 바꾸지 않았다.
- [통합 사후 증거](../../deployments/2026-09-11-7585b16-postchecks.json)에 원본별 해시·백업·감사·원장·HTTP 결과와 한계를 묶었다. 유료 독립검수는 사용자 요청으로 보류했고 기기 확대/물리 터치 미완료도 후속 검수로 유지한다. 영어 번역은 제외한다.
- 배포 후속 목적의5분 자동화는 적용·사후검증 완료 후 종료했다. 아래 과거 미배포/실패 기록은 당시의 증거로 보존하며 현재 상태를 덮어쓰지 않는다.
