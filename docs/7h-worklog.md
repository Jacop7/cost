...
# 라운드 — 가이드 절차 복귀 (2026-08-19)

가이드 §5 재확인 결과, 앞서 "남은 넷"으로 꼽았던 것 중 **둘이 P2(구현 금지)** 였다.

| 항목 | 판정 |
|---|---|
| ORD-04 레시피 계산기(E6) | **P2 — 2차 범위. 구현하지 않는다** |
| 푸시 알림 발송 | **P2 — 구현하지 않는다** |
| 월매출 출처 이원화 | 미션화 (아래 M-030) |
| 로그인 화면 | 후보 — 1차 사이클이 dev 자동로그인으로 완결되므로 P0 아님 |

로스율 설계 변경(사용자 입력 제거 → 실적 측정)은 사용자가 **보류**로 지정. 전제 미션 생성 금지.

### M-030 고정지출률 분모(수기 월매출)와 실제 매출의 괴리를 화면에 드러내기
- 유형: 기능완결
- 발견 근거:
  - `docs/source-v3/레시피_상세로직_유저시나리오_v3.txt:111-113` — "총 월매출 | 고정지출률의 분모. (향후 POS/채널 연동 자동화 여지)"
    → 수기 입력은 **설계 의도**다. 자동 대체가 아니라 **비교 제공**이 맞는 방향.
  - `apps/mobile/src/features/my/hooks.ts:196` — `fixed_costs_monthly.total_revenue` 만 읽는다
  - `apps/mobile/src/features/my/screens/FixedCostScreen.tsx:42` — 실적과 대조하는 코드가 없다
  - 실측(2026-08, 19/31일 경과):
    ```
    수기 입력      12,000,000
    실적(현재)     12,552,000   → 월 환산 20,479,579   괴리 +70.7%
    고정지출률     31.30% (수기)  vs  18.34% (실적 환산)
    제육볶음 개당 고정비 차이 1,555원 → 순이익이 그만큼 과소 계상
    ```
- 사용자 가치: 사장님이 월초에 한 번 적은 예상 매출이 실제와 크게 어긋나도 앱이 알려주지
  않는다. 그 숫자가 **전 메뉴 순이익**에 곱해지므로, 모르는 채로 모든 메뉴 손익이 틀어진다.
- 위험: 손익
- 의존 미션: 없음
- 예상 시간: 45분
- 범위: 수기 월매출 옆에 실적·월 환산·괴리를 보여주고, 원하면 실적으로 채우게 한다.
- 비범위:
  - **고정지출률 계산식은 바꾸지 않는다.** 분모는 여전히 수기 `total_revenue` 다.
    (문서가 그렇게 정의했고, 검산 31.3% 가 여기 걸려 있다)
  - 자동으로 덮어쓰지 않는다. 채우기는 사장님이 누를 때만.
  - 월 환산을 고정지출률에 쓰지 않는다 — 월초 3일차에 분모가 폭주한다.
- 수용 기준:
  - Given 8월 수기 12,000,000 · 실적 12,552,000(19/31일)
    When 고정지출 화면 진입
    Then 실적·월 환산(20,479,579)·괴리(+70.7%)와 "실적 기준이면 18.34%" 가 함께 보인다
  - 예외 1: 판매 기록이 0건인 달 → 괴리를 계산하지 않고 "아직 매출 기록이 없어요"
  - 예외 2: 지난달(월 완료) → 월 환산 = 실적 그대로(일할 추정 안 함)
  - 예외 3: 수기 값이 0/미입력 → 비율 null, 괴리 표시 안 함, 채우기 버튼은 활성
  - 불변: `fixed_cost_rate()` 반환값, 제육볶음 33.49%, 고정지출률 31.30%
- 검증 방법: 실 DB 쿼리로 4개 케이스 재현 + core 회귀 + 검산 3종
- 상태: READY
- 후속 미션: 감사 결과 반영 후 재탐색

#### M-030 구현 전 검증서
- 근거 문서/절: 레시피 v3 §3.3(총 월매출=고정지출률 분모), §136-137(31.3% 예시), 가이드 §5 P0-5
- 현재 동작 재현: 위 실측 표. 두 화면이 각각 수기/실적을 보여주며 서로를 언급하지 않는다.
- 확인된 문제와 증거: 괴리 70.7%, 개당 고정비 1,555원 차이 (실 DB 쿼리)
- 정상 흐름 기대값: 화면이 수기·실적·환산·괴리·환산기준 비율을 한 카드에 보여준다
- 예외별 기대값: 위 예외 1~3
- 변경되어야 하는 값: 화면 표시(정보 추가), `total_revenue`(사장님이 채우기를 누른 경우만)
- 변하면 안 되는 값: `fixed_cost_rate()` 공식·반환, 레시피 손익, 검산 3종, 타 매장
- 상류/하류 영향: 상류 = daily_sales(E10). 하류 = E4 → 전 레시피 손익 → profit_trends
  (단 채우기를 눌러 total_revenue 가 바뀔 때만 발생하며, 이는 기존 E4 경로 그대로다)
- UI 상태·문구 계약: "예상"과 "실적"을 명시적으로 구분. 자동 반영이 아님을 문구로 밝힌다.
- 선행 실패 테스트: 현재 화면에 실적 관련 요소가 0개임을 grep 으로 확인(위 근거)
- 남은 불확실성: 없음 — 계산식 불변이므로 검산 위험 없음
- 구현 준비 판정: **READY**

#### M-030 구현 후 재검증 — **완료**
- 선행 조건 재현: 화면에 실적 관련 요소 0개 → 이제 두 화면(FixedCost·FixedCostEdit)에 비교 카드
- 수용 기준 전건:

| 케이스 | 기대 | 실제 | 판정 |
|---|---|---|---|
| 정상 (8월, 19/31일) | 실적·환산·괴리·환산기준 비율 표시 | 12,552,000 / 20,479,579 / +70.66% / 18.34% | ✅ |
| 예외1 판매 0건 (2026-01) | 괴리 계산 안 함 | `has_sales:false`, gap `null` | ✅ |
| 예외2 지난달 (7월) | 월 환산 = 실적 (일할 추정 안 함) | `in_progress:false`, projected = actual = 2,116,000 | ✅ |
| 예외3 수기 미입력 | 비율 null, 괴리 표시 안 함 | `manual_revenue:null`, `rate_manual:null` | ✅ |

- 불변 계약 전건:

| 항목 | 기대 | 실제 |
|---|---|---|
| 대파 기준단가 | 4.7059 | 4.7059 |
| 제육볶음 재료비 | 2,834.55 | 2,834.55 |
| 제육볶음 순이익 | 4,018.54 | 4,018.54 |
| 제육볶음 순이익률 | 33.49% | 33.49% |
| 고정지출률 | 31.30% | 31.30% |
| RPC 오버로드 | 0 | 0 |

- 회귀: core 154 통과 · mobile typecheck 0 · 웹 번들 성공
- 상하류 재실행: `fixed_cost_revenue_check` 는 조회 전용이라 E4 경로를 건드리지 않는다.
  '채우기'는 입력칸 값만 바꾸고 저장은 기존 `save_fixed_costs` → E4 경로 그대로.
- 판정: **완료**

#### M-030 에서 파생된 관찰
`fixed_cost_revenue_check` 로 지난달(2026-07)을 조회하니 실적이 2,116,000 인데 수기가
12,000,000 이었다. 시드가 3주치만 만들어 7월이 3일치뿐인 탓이며 제품 결함은 아니다.
다만 **월 전체 데이터가 없는 달에도 수기 값이 그대로 비율을 만든다**는 사실은 드러났다 —
지난달 고정지출률이 31.3% 로 남아 있고, 실적 기준이면 177.5% 다. 화면이 이제 그걸 보여준다.

---

### M-031 · DB 회귀 스위트 (public 함수 66개에 테스트 0개)

- 근거: 가이드 §5 P0-6 "E7 전후 재고, 기준단가, 재고 이벤트 수가 변하지 않는 테스트를
  **반드시 둔다**" / P0-2 "이미 입고 확정된 주문을 다시 확정해도 이중 반영되지 않도록
  **DB 수준에서 방어**한다" / 불변식 8
- 현재 동작 재현: `public` 함수 66개, `packages/db/tests` 없음. 0025~0039 마이그레이션 15개를
  손으로 친 SQL 로만 검증했다. 0027 은 그렇게 **조용히 실패**했다(RLS 에 DELETE 정책이 없어
  `delete from inventory_events` 가 0행 → 되돌림 실패 → "팔았는데 재고가 도로 가득 찼다").
- 변경되어야 하는 값: 없음(테스트 추가). 단 0040 은 표식 1개를 바로잡는다.
- 변하면 안 되는 값: 검산 3종, RPC 오버로드 0, core 154
- 구현 준비 판정: **READY** — 새 라이브러리 없이 psql `ON_ERROR_STOP` 으로 충분(가이드 P2)

#### M-031 구현 후 재검증 — **완료**

| 파일 | 무엇을 못 박았나 |
|---|---|
| `01_checksums.sql` | 대파 4.7059 · 제육볶음 2,834.55/4,018.54/33.49% · 고정지출률 31.30% · 손익 항등식 |
| `02_e7_immutable.sql` | E7 전후 재고·기준단가·이벤트 수·**단가추이 수** 불변 (가이드 P0-6) |
| `03_e1_inbound.sql` | 재고 증가 = 개수 × 용량 · 같은 키/키 없이 재확정 모두 이중 반영 없음 (P0-2) |
| `04_ledger.sql` | 18종 전부 원장 합계 = 재고 · 시점별 잔고 음수 0 · seq 결측 0 |
| `05_loss_ledger.sql` | 0038 6건 회귀 · 되돌림 멱등 · 로스율 0~100 |
| `06_sales_consume.sql` | 목표치 대조: 10→15 는 차이만, 15→3 은 원복, 취소해도 원장 보존 |
| `07_guards.sql` | 남의 매장 쓰기 42501 · 필수값/중복/자기참조 · 사용 중 삭제 23503 |

- 실행: `pnpm db:test` (루트) 또는 `node tests/run.mjs [번호]`
- 결과: **7/7 통과, 단언 60여 개, 1.0초**
- 백지 검증: `supabase db reset` 으로 마이그레이션 41개 재적용 후에도 7/7
- 빨강→초록 증명: `real_loss_rate` 에서 "되돌림 제외" 조건을 일부러 빼자
  `05` 가 "되돌린 뒤 실측 로스율이 null 로 복귀"에서 실패 → 복구 후 재통과
- 회귀: core 154 통과 · mobile typecheck 0

#### M-031 이 즉시 잡아낸 결함 → 마이그레이션 0040

`04_ledger.sql` 첫 실행에서 **판매 소진 이벤트 814건이 `unit_normalized = false`** 로 드러났다.
`reconcile_sales_consumption`(0028)이 0034 보다 먼저 쓰여 이 컬럼을 채우지 않는다.
값 자체는 기준단위가 맞다(18종 전부 원장 합계 = 재고 통과). **표식만 거짓이었다.**

위험한 이유: 0034 는 `and not ev.unit_normalized` 로 재실행을 막는다. 같은 패턴의 보정
마이그레이션이 하나만 더 들어와도 이 814건에 `per_volume` 을 한 번 더 곱해 원장이 통째로
깨진다. 잠복이지만 터지면 복구가 어렵다.

0040: 전건 true 로 보정 + 컬럼 기본값을 true 로 변경(컬럼을 빠뜨린 미래의 writer 도 옳은 값을
갖는다) + 컬럼 뜻을 "count_delta 가 기준단위임"으로 주석에 고정.

#### M-031 에서 파생된 관찰과 후속 미션

**① 로스율 방향성은 여전히 미결이다 (사장님 결정 대기)**
`05` 를 쓰다가 재현됐다 — 대파 1,940g 폐기 → 실측 로스 0.400% → 기준단가 **4.7059 → 4.0161
(−14.7%)**. 0038 은 산수만 고쳤고, "측정값이 추정치를 통째로 대체한다"는 의미론은 그대로다.
테스트는 어느 편도 들지 않고 "어느 쪽으로 정하든 산수는 맞아야 한다"만 지킨다 —
`05_loss_ledger.sql` 머리말에 미결이라고 명시했다.

**② M-032 · 재료 없는 메뉴가 손익 그래프에 가짜 봉우리를 만든다**
`profit_trends` 에 재료비율 0.00 · 순이익률 57.11/88.41 인 스냅샷이 저장돼 있다.
`recompute_recipe` 는 정상이다(원인별로 같은 값을 낸다 — 확인함). 원인은 `save_recipe` 의
`if p_payload ? 'lines'` 다: 재료 없이 메뉴만 저장하면 재료비 0 인 스냅샷이 그대로 적재된다.
절대원칙 4 때문에 기존 행은 지울 수 없으므로, 앞으로 찍히지 않게 막는 쪽으로 처리해야 한다.

**③ 감사에서 의심했다가 무혐의로 확인된 것 (기록)**
- 저장 버튼 중복 탭 방어 — 전 화면 있음
- 구매옵션 삭제 시 입고 이력 파손 — `order_records` 가 `volume/amount` 스냅샷이라 안 깨짐
- 반제품 비활성화 시 부모 원가 붕괴 — 전개가 `active` 를 보지 않아 정상

---

### M-033 · 로스율 제거 — 추정을 버리고 실제 폐기만 센다
...
| | 19일간 | 월 환산 |
|---|---|---|
| 로스율이 원가에 얹던 금액 | 32,631원 | **53,240원** |
| 실제 기록된 폐기 (식재료 1,336 + 메뉴 17,007) | 18,343원 | 29,930원 |

로스율이 기록된 폐기 전부보다 **1.8배** 많은 비용을 잡고 있었다.

폐기는 셋인데 결정은 둘을 덮는다.
① 손질 손실 — 15% 추정이 대신 서 있던 것. **1차 범위 밖으로 확정.**
② 보관 폐기(E2) · ③ 조리 폐기(qty_waste) — 사장님이 직접 입력.

#### 감수하기로 한 것 (명시)
- 월 53,240원(실측)이 원가에 잡히지 않는다.
- 장부 재고가 물리 재고보다 많아진다(소진할 때마다 다듬어 버린 만큼 벌어진다).
- 2차에서 입고 실측량 입력으로 닫을 수 있다. `CLAUDE.md` 절대원칙 5에 적어 뒀다.

#### 검산 기준값 변경

| | 이전 | 이후 |
|---|---|---|
| 대파 기준단가 | 4.7059원/g | **4.0000원/g** |
| 제육볶음 재료비 | 2,834.55 | **2,806.40** |
| 제육볶음 순이익 | 4,018.54 | **4,046.69** |
| 제육볶음 순이익률 | 33.49% | **33.72%** |
| 고정지출률 | 31.30% | 31.30% (불변) |

전 메뉴 +0.15 ~ +0.77%p. 로스를 지던 재료의 원가 비중이 작아 영향은 작다.

#### 마이그레이션 0041
- `base_unit_price` 에서 `÷ (1 − 로스율)` 제거 — 실입고량 가중평균 그대로
- `real_loss_rate()` 삭제, `ingredients.loss_rate` · `categories.default_loss_rate` 컬럼 삭제
- 이 컬럼을 참조하던 함수 7개 재작성(`ingredient_list` 는 반환 컬럼이 줄어 drop 후 재생성)
- **조리 폐기를 원장에서 분리** — `inventory_events.waste` 추가.
  판매분은 `consume`, 조리 폐기분은 `discard`. `reconcile_sales_consumption` 이
  두 목표치를 따로 대조한다. 이전에는 "제육볶음 18개 소진 (폐기 2개 포함)" 한 줄이었다.
- `sales_summary` 의 `waste_ingredient` 에 `sales_item_id is null` 조건 — 없으면
  조리 폐기가 원장과 매출 양쪽에서 잡혀 이중 계산된다.

#### 0041 이 도중에 잡아낸 결함 2건
1. **되돌린 폐기가 월 손익에 남아 있었다.** 0038 이 `real_loss_rate` 에서만 되돌림을
   제외하고 `sales_summary` 는 놓쳤다. 폐기 7,760원을 취소해도 손익의 폐기 손실은
   그대로였다(실측 재현). 같은 마이그레이션에서 고쳤다.
2. **`core/inventory.ts` 의 `remainConverted` 만 재고에 (1−로스)를 곱하고 있었다.**
   SQL `stock_total_base()` 는 곱하지 않는다 — 절대원칙 3(두 곳 공식이 어긋나면 안 됨)
   위반이었다. 재고는 물건의 양이지 쓸 수 있는 양의 추정치가 아니다.

#### 동기화 범위
seed(로스 키 31곳) · `CLAUDE.md`(절대원칙 5 신설 + 검산값) · `core/pricing.ts` ·
`core/inventory.ts` · core 테스트 4종 · DB 테스트 01/05 · `types/entities.ts` ·
앱 6개 파일(식재료 폼의 로스율 입력 UI, 상세 표시, 카테고리 기본 로스율, 입고 미리보기)

#### 재검증
- `supabase db reset` 으로 마이그레이션 42개 재적용 → DB 스위트 **7/7**
- core **136 통과** · mobile typecheck **0**
- `05_loss_ledger.sql` → `05_discard.sql` 로 재작성.
  "폐기해도 기준단가 불변" 을 못 박아 사장님이 지적한 역전이 다시는 못 생기게 했다.

#### M-033 후속 · 로스율을 표시 전용으로 되살림 (0042)

사장님: "로스율은 식재료 상세페이지에 표시는 되어야 하는데 폐기이력이 함께 나오면서"

로스율이 **원가를 조용히 움직이는 값**에서 **얼마나 버리고 있는지 알려주는 값**으로 역할이 바뀌었다.
`ingredient_loss()` 는 어디에도 곱해지지 않는다 — 0041 의 핵심을 지키면서 숫자만 돌려준다.

- 보관 폐기(상해서 버림)와 조리 폐기(만들었는데 못 팖)를 **갈라서** 보여준다.
  한 숫자로 뭉치면 발주를 줄여야 하는지 덜 만들어야 하는지 알 수 없다.
- 폐기 0건이면 `rate` 는 null — 0% 로 단정하지 않는다("안 버렸다"와 "아직 모른다"는 다르다).
- 되돌린 폐기는 빠진다. 분모는 실입고량(0038 에서 배운 규칙 그대로).
- `stock_history` 가 `waste` 를 실어 화면이 '보관 폐기'/'조리 폐기'로 다르게 부른다.
- `LossCard` 를 폐기 이력 바로 위에 뒀다 — 숫자만 보면 어디서 나온 값인지 모른다.

#### 0042 가 잡아낸 결함 · 조리 폐기 되돌리기가 원장을 깨뜨렸다

재고 내역 화면에서 조리 폐기를 되돌릴 수 있었는데, 그 주인은 **매출 기록**이다. 실측:

```
① 판매 후 재고            2,190.0
② 조리 폐기 되돌림        2,265.0  (+75g)
③ 매출을 같은 값으로 재저장 2,265.0  ← 2,190 으로 돌아와야 하는데 안 돌아옴
```

매출은 "3개 버렸다"인데 재고는 반영 안 된 채로 굳는다. 원인은 `e2_discard_reverted` 가 만드는
보정 이벤트에 `waste` 표시가 없어 목표치 대조가 판매분 몫으로 잘못 세는 것이다.
DB 에서 거부(`sales_item_id is not null` → 22000)하고 UI 에서도 되돌리기를 감췄다.
사장님이 고쳐야 할 곳은 그 날 매출이라고 안내한다.

재검증: 백지 reset 후 DB **7/7** · core **136** · typecheck **0**

## 2026-08-20 · 재고 분할 모델 제거 (0056)

기존 `sealed_count + opened_remain` 모델은 화면에서 별도로 입력·관리하는 흐름이 없어 폐기했다.
`20260820000056_single_stock_total.sql`에서 기존 값을 기준단위 총량으로 합친 뒤
`inventory_states.stock_total` 하나로 전환했다. E1 입고, E2/E8 차감, 복구, E5 실사와
상세 조회는 모두 이 총량을 직접 사용한다. 위의 분할 재고 관련 기록은 당시 구현 이력이며
현재 스키마에는 적용되지 않는다.
